...
var BACKEND_PORT = process.env.BACKEND_PORT ?? "10000";
var BACKEND_URL = `http://127.0.0.1:${BACKEND_PORT}`;
async function resolveFilePathRemote(rawPath, projectRoot, context) {
  const candidates = [];
  ...
  tools: (tool) => [
    tool({
      ...
      parameters: typebox_exports.Object({
        filepath: typebox_exports.String({ description: "Path to the configuration file to preview." })
      }),
      async execute(params, _config, context) {
        ...
        try {
          ...
          if (absolutePath) {
            ...
            filepath = filepath.replace(/^\/sandbox\//, "");
          }
