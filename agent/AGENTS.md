...
- **Running Simulations**: To execute a simulation, construct the finalized configuration file directly inside the appropriate configuration directory (`../configs/` or `/sandbox/configs/`), present it to the user via `file_open`, wait for explicit user approval, and then call `run_simulation` (`config_filepath`, `run_descriptor`, `session_path`). The `run_simulation` tool handles creating the run directory in the appropriate logs directory (`../logs/...` or `/sandbox/runs/...`), staging `config.cfg`, running BookSim sequentially, and returning parsed metrics for your response in chat. Do NOT show base template configs asking "how about we edit this".
