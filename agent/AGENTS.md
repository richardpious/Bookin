...
- **Running Simulations**: Before running simulation, create the appropriate folder in `../logs/<username>/<session>/run_<n>_<topology>_<descriptor>` (e.g., `run_01_mesh4x4_uniform` or `run_02_mesh4x4_rate0.05`, where `<descriptor>` is whatever parameter is unique to that run) and place/write the complete, finalized config file for the simulation there. Do NOT use existing config files as a crutch or show base configs asking "how about we edit this".
