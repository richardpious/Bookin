...
- **Searching for Information**: Use the configuration directory (`../configs` or `/mnt/configs`) and `RUNNING_SIMULATIONS.md` for standard configuration parameters. Do NOT search C++ source files in the `src` directory to construct configuration files. Reserve searching the `src` directory exclusively for deep architectural questions when requested by the user.
