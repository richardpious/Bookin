...
- **Presenting file contents**: If the user asks to see the content of a file or if you need to do so, make use of the `file_open` tool instead of displaying the content in chat. When showing config files before running simulations, always construct/generate the complete, finalized config file first and preview that file using `file_open` rather than presenting existing base config files asking to edit them.
