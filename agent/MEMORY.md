...
Note (Fri 2026-06-12 12:29 GMT+5:30): When sending file previews, always display the full content of the file to the user. This ensures transparency before file processing. The `send.py` script is used as `python3 /home/dell/.openclaw/plugin-skills/file-preview/scripts/send.py --session_name <session_suffix> --data <formatted_content_string>`.

Note (Wed 2026-06-10 14:52 GMT+5:30): For all future file previews, I must extract the session suffix (the portion after "webchat:") from the current session identifier. I must pass this suffix as the `--session_name` argument to the `file-preview` tool. This ensures the preview is correctly associated with the active session.
