...
- **Log Organization (Mon 2026-06-08, Mon 2026-06-22, Tue 2026-07-07):** All simulation logs and artifacts must be stored strictly according to the structure: `../logs/<session>/<run>`. When making the session folder, use the exact identifier for the current session (e.g., `sesname`) and ensure it matches the actual runtime session ID. You must also have a copy of the config file used to run the simulation in the folder where the run outputs are stored.
