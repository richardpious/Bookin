...
export class ConnectionManager {
    ...
    async connect(url, params) {
        ...
        try {
            ...
            const handshakeComplete = new Promise((resolve, reject) => {
    ...
    async performHandshake() {
        try {
            ...
            this.handshakeCompleters?.reject(error instanceof Error ? error : new Error('Handshake failed'));
            this.handleDisconnect();
        }
    }
    /**
    ...
    handleMessage(data) {
        try {
            ...
            if (data.length > MAX_MESSAGE_SIZE) {
                throw new Error(`Incoming message exceeds maximum size of ${MAX_MESSAGE_SIZE} bytes (received: ${data.length} bytes)`);
            }
            ...
            const dataPreview = data.length > 100 ? `${data.slice(0, 100)}... (${data.length} bytes total)` : data;
     ...
     */
    handleError(message, original, recoverable = false) {
        const event = {
            message,
            original,
            recoverable,
 ...
 * ```
 */
export function createConnectionManager(transport, config) {
