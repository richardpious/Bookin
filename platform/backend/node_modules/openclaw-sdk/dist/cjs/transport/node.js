...
exports.createNodeWebSocketTransport = createNodeWebSocketTransport;
...
class NodeWebSocketTransport {
    ...
    constructor(config) {
        this.ws = null;
        // Ready state
        this._readyState = websocket_js_1.ReadyState.CLOSED;
        this.config = config;
    }
    /**
    ...
    get readyStateString() {
        return (0, websocket_js_1.readyStateToString)(this._readyState);
    }
    /**
     ...
     */
    async connect(url) {
        if (this._readyState !== websocket_js_1.ReadyState.CLOSED) {
            throw new Error('Cannot connect: transport is not closed');
        }
        this._readyState = websocket_js_1.ReadyState.CONNECTING;
        return new Promise((resolve, reject) => {
            try {
                ...
                this.ws = new ws_1.WebSocket(url, options);
                // Set up event handlers
                this.ws.on('open', () => {
                    this._readyState = websocket_js_1.ReadyState.OPEN;
                ...
                this.ws.on('error', (error) => {
                    if (this._readyState === websocket_js_1.ReadyState.CONNECTING) {
                        reject(error);
                    }
     ...
     */
    send(data) {
        if (this._readyState !== websocket_js_1.ReadyState.OPEN || !this.ws) {
    ...
    close(code, reason) {
        if (this.ws) {
            ...
            this._readyState = websocket_js_1.ReadyState.CLOSING;
        }
    }
}
 ...
 * ```
 */
function createNodeWebSocketTransport(config) {
    return new NodeWebSocketTransport(config);
}
// ============================================================================
// Re-exports
// ============================================================================
exports.default = NodeWebSocketTransport;
