...
exports.ReconnectManager = exports.DEFAULT_RECONNECT_CONFIG = void 0;
exports.createReconnectManager = createReconnectManager;
...
class ReconnectManager {
    constructor(config = {}) {
        ...
        this.config = { ...exports.DEFAULT_RECONNECT_CONFIG, ...config };
    }
    /**
     * Get current state.
     */
    ...
    isTerminalAuthError(error) {
        if (error instanceof errors_js_1.ReconnectError) {
            return TERMINAL_AUTH_ERRORS.includes(error.code);
        }
        return false;
    }
    /**
    ...
    isRetryableAuthError(error) {
        if (error instanceof errors_js_1.ReconnectError) {
            return RETRYABLE_AUTH_ERRORS.includes(error.code);
        }
        return false;
    }
    /**
     ...
     */
    async reconnect(connectFn, refreshTokenFn) {
        this.aborted = false;
        this.attempt = 0;
        this.authRetries = 0;
        ...
        while (!this.aborted && this.attempt < this.config.maxAttempts) {
            ...
            try {
                ...
                if (this.isTerminalAuthError(err)) {
                    ...
                    throw new errors_js_1.ReconnectError({
 ...
 * ```
 */
function createReconnectManager(config) {
