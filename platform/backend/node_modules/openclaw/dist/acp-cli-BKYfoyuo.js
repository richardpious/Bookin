import { a as normalizeLowercaseStringOrEmpty, c as normalizeOptionalString, p as readStringValue, s as normalizeOptionalLowercaseString, t as hasNonEmptyString } from "./string-coerce-DKw2K5wM.js";
import { y as resolveIntegerOption$1 } from "./number-coercion-DjYijOzx.js";
...
import { c as isRecord, g as shortenHomePath } from "./utils-DlHt_n1Q.js";
import { n as VERSION } from "./version-BjFOZC7j.js";
import "./path-guards-CBe_wA_B.js";
import { r as readJsonFile, s as writeTextAtomic } from "./json-files-ruLzKqUC.js";
import { n as defaultRuntime } from "./runtime-yzlkhCoS.js";
import { a as routeLogsToStderr } from "./console-B-lIqwpI.js";
import { i as getRuntimeConfig } from "./io-By9euW-h.js";
import { i as omitEnvKeysCaseInsensitive, n as listKnownProviderAuthEnvVarNames } from "./provider-env-vars-wzQqSStL.js";
import { t as sanitizeTerminalText } from "./safe-text-J_0sTthZ.js";
import { l as BASE_THINKING_LEVELS } from "./thinking-Cq-IXzst.js";
import { o as withFileLock } from "./file-lock-C4JGV-yb.js";
import "./file-lock-BDrW-jMX.js";
import "./config-Bzaup9wC.js";
import { i as GATEWAY_CLIENT_NAMES, r as GATEWAY_CLIENT_MODES, t as GATEWAY_CLIENT_CAPS } from "./client-info-ErIekSPI.js";
...
import { r as isKnownCoreToolId } from "./tool-catalog-Bxn5jw8h.js";
import { a as resolveWindowsSpawnProgram, r as materializeWindowsSpawnProgram } from "./windows-spawn-CdrXWu4L.js";
import { r as isMutatingToolCall } from "./tool-mutation-DIqK0fbR.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-DkbKZvAw.js";
import { t as resolveGatewayClientBootstrap } from "./client-bootstrap-CAzmsf0W.js";
import { t as readSecretFromFile } from "./secret-file-DC4FEMh6.js";
import { t as resolveGatewayAuthOptions } from "./gateway-secret-options-DnFu8C-V.js";
...
//#endregion
//#region src/acp/numeric-options.ts
...
const DEFAULT_MAX_SESSIONS$1 = 200;
...
function normalizeLedgerOptions(options = {}) {
	return {
		maxSessions: resolveIntegerOption(options.maxSessions, DEFAULT_MAX_SESSIONS$1, { min: 1 }),
...
function createFileAcpEventLedger(params) {
	...
	const ensureParentDir = async () => {
		await fs$1.mkdir(path.dirname(params.filePath), {
			recursive: true,
			mode: 448
		});
	};
...
//#endregion
//#region src/acp/session-lineage-meta.ts
...
async function getAvailableCommandsForAcp() {
	acpCommandsModulePromise ??= import("./commands-BxmKXmdL.js");
...
var AcpGatewayAgent = class {
	...
	async newSession(params) {
		...
		await this.startLedgerSession(session, {
			complete: true,
			reset: true
		});
	...
	async loadSession(params) {
		...
		const listedLedgerReplay = !hasExplicitRouting && !exactLedgerReplay.complete ? await this.readLedgerReplayBySessionKey(params.sessionId) : {
			complete: false,
			events: []
		};
		...
		const ledgerReplay = exactLedgerReplay.complete && exactLedgerReplay.sessionKey === sessionKey ? exactLedgerReplay : listedLedgerReplay.complete && listedLedgerReplay.sessionKey === sessionKey ? listedLedgerReplay : await this.readLedgerReplay({
			sessionId: params.sessionId,
			sessionKey
		});
		const session = this.sessionStore.createSession({
	...
	async listSessions(params) {
		...
		while (true) {
			...
			if (rows.length > end || result.hasMore !== true || fetchLimit >= ACP_LIST_SESSIONS_MAX_FETCH_LIMIT) break;
	...
	async emitSessionUpdate(params) {
	...
	async replaySessionTranscript(sessionId, transcript) {
		for (const message of transcript) {
			...
			for (const chunk of replayChunks) await this.emitSessionUpdate({
				sessionId,
				update: {
	...
	async replayLedgerSession(sessionId, ledgerReplay) {
		for (const event of ledgerReplay.events) await this.emitSessionUpdate({
			sessionId,
			update: event.update,
			record: false
		});
	...
	async sendSessionSnapshotUpdate(session, sessionSnapshot, options) {
		...
		if (sessionSnapshot.metadata) await this.emitSessionUpdate({
			sessionId: session.sessionId,
			sessionKey: session.sessionKey,
			...session.ledgerSessionId ? { ledgerSessionId: session.ledgerSessionId } : {},
			runId: options.runId,
		...
		if (sessionSnapshot.usage) await this.emitSessionUpdate({
			sessionId: session.sessionId,
			sessionKey: session.sessionKey,
			...session.ledgerSessionId ? { ledgerSessionId: session.ledgerSessionId } : {},
			runId: options.runId,
...
async function serveAcpGateway(opts = {}) {
	...
	const eventLedger = createFileAcpEventLedger({ filePath: resolveDefaultAcpEventLedgerPath(process.env) });
