...
import { a as readNetworkInterfaces, i as pickMatchingExternalInterfaceAddress, n as pickPrimaryTailnetIPv4, o as safeNetworkInterfaces } from "./tailnet-CyU_ckBA.js";
import { f as isLoopbackIpAddress, h as normalizeIpAddress, i as isCanonicalDottedDecimalIPv4, p as isPrivateOrLoopbackIpAddress, s as isIpInCidr } from "./ip-Y63B6JB6.js";
...
function resolveLocalInterfaceAddressMatch(ip, snapshot) {
	const normalized = normalizeIp(ip);
	if (!normalized) return false;
	const effectiveSnapshot = arguments.length >= 2 ? snapshot : safeNetworkInterfaces();
	if (!effectiveSnapshot) return;
	for (const entries of Object.values(effectiveSnapshot)) for (const entry of entries ?? []) if (normalizeIp(entry.address) === normalized) return true;
	return false;
}
/**
...
function isSecureWebSocketUrl(url, opts) {
	...
	if (isTrustedPlaintextWebSocketHost(parsed.hostname)) return true;
...
//#endregion
export { resolveRequestClientIp as _, isPrivateOrLoopbackAddress as a, isTrustedProxyAddress as c, pickPrimaryLanIPv4 as d, resolveClientIp as f, resolveLocalInterfaceAddressMatch as g, resolveHostName as h, isLoopbackHost as i, isValidIPv4 as l, resolveGatewayListenHosts as m, isLocalishHost as n, isPrivateOrLoopbackHost as o, resolveGatewayBindHost as p, isLoopbackAddress as r, isSecureWebSocketUrl as s, defaultGatewayBindMode as t, normalizeHostHeader as u };
