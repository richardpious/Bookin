...
function createOpenRouterStreamDeadline(timeoutMs) {
	return {
...
function resolveOpenRouterStreamRemainingMs(deadline) {
	...
	if (remainingMs <= 0) throw new Error(`OpenRouter music generation timed out after ${deadline.timeoutMs}ms`);
	...
	try {
		return await Promise.race([reader.read(), new Promise((_, reject) => {
			timeoutId = setTimeout(() => {
				reject(/* @__PURE__ */ new Error(`OpenRouter music generation timed out after ${deadline.timeoutMs}ms`));
			}, timeoutMs);
		})]);
	} catch (error) {
...
const openRouterMusicTestInternals = { readOpenRouterAudioStream };
//#endregion
