...
import { t as addGatewayServiceCommands } from "./register-service-commands-BmYMtCOF.js";
import { n as defaultRuntime } from "./runtime-yzlkhCoS.js";
import { t as formatHelpExamples } from "./help-format-DXVAk021.js";
import { t as addGatewayRunCommand } from "./run-command-BRm7VtFO.js";
...
const gatewayStatusModuleLoader = createLazyImportLoader(() => import("./gateway-status-D7_TA2-6.js"));
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-yHEdqv81.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(() => import("./bonjour-discovery-DUT95xqX.js"));
const wideAreaDnsModuleLoader = createLazyImportLoader(() => import("./widearea-dns-Cl7X68K-.js"));
const healthStyleModuleLoader = createLazyImportLoader(() => import("./health-style-DgsRgSYJ.js"));
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-DfIPDmP4.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(() => import("./diagnostic-stability-bundle-wBCOrm7K.js"));
const supportExportModuleLoader = createLazyImportLoader(() => import("./diagnostic-support-export-std1OGbI.js"));
const daemonStatusGatherModuleLoader = createLazyImportLoader(() => import("./status.gather-B3bzANr1.js"));
...
async function callGatewayCli(method, opts, params) {
	return (await import("./call-C6t3f06S.js")).callGatewayCli(method, opts, params);
}
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayTransportErrorJson } = await import("./call-BFQSeMzC.js");
...
function registerGatewayCli(program) {
	...
	gatewayCallOpts(gateway.command("stability").description("Fetch payload-free Gateway stability diagnostics").option("--limit <limit>", "Maximum number of recent events", "25").option("--type <type>", "Filter by diagnostic event type").option("--since-seq <seq>", "Only include events after this sequence").option("--bundle [path]", "Read a persisted stability bundle instead of calling Gateway; pass \"latest\" for newest").option("--export", "Write a shareable support diagnostics export", false).option("--output <path>", "Diagnostics export output .zip path").action(async (opts, command) => {
		await runGatewayCommand(async () => {
			const { normalizeDiagnosticStabilityQuery, selectDiagnosticStabilitySnapshot } = await import("./diagnostic-stability-UnYWsgfi.js");
			const rpcOpts = resolveGatewayRpcOptions(opts, command);
	...
	gateway.command("discover").description("Discover gateways via Bonjour (local + wide-area if configured)").option("--timeout <ms>", "Per-command timeout in ms", "2000").option("--json", "Output JSON", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			const [{ readSourceConfigBestEffort }, { discoverGatewayBeacons }, { resolveWideAreaDiscoveryDomain }, { dedupeBeacons, parseDiscoverTimeoutMs, pickBeaconHost, pickGatewayPort, renderBeaconLines }, { withProgress }] = await Promise.all([
				...
				import("./discover-CaHatExh.js"),
				import("./progress-BHo4iFAp.js")
			]);
