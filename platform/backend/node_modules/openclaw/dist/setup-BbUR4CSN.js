import { d as resolveGatewayPort } from "./paths-BQQu4M8v.js";
...
import { d as normalizeSecretInputString } from "./types.secrets-CuOz9KrZ.js";
...
import { n as t } from "./i18n-eLAkCRYg.js";
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-D08lJVx6.js";
import { a as stripPendingPluginInstallRecords, i as hasPendingPluginInstallRecords, n as commitConfigWriteWithPendingPluginInstalls, s as unchangedPendingPluginInstallRecordIds } from "./plugins-install-record-commit-CxhwUu3W.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-B1iPBk9A.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-CWhPS5Ld.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-BJEmTlTU.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-CyoJZTm7.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-mtXlAlIT.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-cki7A7gj.js"), import("./setup-registry-C-j3Wny5.js")]);
...
async function runSetupWizard(opts, runtime, prompter) {
	runtime ??= defaultRuntime;
	const onboardHelpers = await import("./onboard-helpers-CZKIvAw7.js");
	...
	const writeSetupConfigFile = async (config, opts = {}) => await writeWizardConfigFile(config, {
		...opts,
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-J10iGtIH.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-CWRV0ERi.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-CWRV0ERi.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-D7wtS-CA.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-CHNIwJzh.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-BQlQWp1C.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-BFKBGiC2.js");
		const { setupChannels } = await import("./onboard-channels-BNuNBvuM.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-CM1IZMVV.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-DGkxiYyd.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-D6o1Ijfd.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-BbjbmpEc.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-CHTWQ8uF.js");
	...
	nextConfig = await writeSetupConfigFile(nextConfig, { allowConfigSizeDrop: configResetPerformed });
	const { finalizeSetupWizard } = await import("./setup.finalize-CifHXGQX.js");
