import { S as resolveTimerTimeoutMs } from "./number-coercion-DjYijOzx.js";
import { i as buildBaseOptions, s as clampThinkingLevel } from "./sanitize-unicode-8KnR9CZq.js";
import { t as AssistantMessageEventStream } from "./event-stream-C9Fn8W_T.js";
import { n as registerSessionResourceCleanup } from "./session-resources-DwYipzEQ.js";
...
import { i as processResponsesStream, n as convertResponsesMessages, s as convertResponsesTools } from "./openai-responses-shared-CQbWtrOu.js";
//#region src/llm/utils/diagnostics.ts
...
//#endregion
//#region src/llm/utils/oauth/openai-codex-jwt.ts
...
//#endregion
//#region src/llm/providers/openai-codex-responses.ts
let os = null;
				...
				try {
					...
					if (attempt < MAX_RETRIES && isRetryableError(response.status, errorText)) {
						...
						if (retryAfterMs !== null) {
							...
							if (/^\d+(?:\.\d+)?$/.test(trimmedRetryAfterMs) && Number.isFinite(millis)) delayMs = Math.max(0, millis);
						} else {
							const retryAfter = response.headers.get("retry-after");
							if (retryAfter) {
								const trimmedRetryAfter = retryAfter.trim();
								const seconds = Number(trimmedRetryAfter);
								if (/^\d+$/.test(trimmedRetryAfter) && Number.isFinite(seconds)) delayMs = Math.max(0, seconds * 1e3);
