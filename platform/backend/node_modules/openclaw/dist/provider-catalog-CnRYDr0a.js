...
import { n as buildManifestModelProviderConfig } from "./provider-catalog-shared-C_pLOB97.js";
...
function clearNvidiaFeaturedModelCacheForTests() {
	featuredModelCache = void 0;
	featuredModelRequest = void 0;
}
async function loadNvidiaFeaturedModels() {
	const now = Date.now();
	if (featuredModelCache && featuredModelCache.expiresAtMs > now) return featuredModelCache.models;
	featuredModelRequest ??= fetchNvidiaFeaturedModels();
	try {
		const models = await featuredModelRequest;
		if (models && models.length > 0) featuredModelCache = {
...
async function fetchNvidiaFeaturedModels() {
	try {
...
function parseNvidiaFeaturedModels(payload) {
	if (!payload || typeof payload !== "object") return null;
	const rows = payload["featured-models"];
