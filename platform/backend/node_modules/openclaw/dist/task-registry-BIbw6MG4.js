...
import { t as isDeliverableMessageChannel } from "./message-channel-normalize-mFW26rOU.js";
import "./message-channel-7236hqCk.js";
import { n as requireNodeSqlite, t as configureSqliteWalMaintenance } from "./sqlite-wal-VSBL7QjO.js";
...
import { n as shouldRouteCompletionThroughRequesterSession } from "./completion-delivery-policy-BOaUlgsw.js";
...
import { a as sanitizeTaskStatusText, i as formatTaskStatusTitleText } from "./task-status-C-pTpHs4.js";
import { n as resolveTaskCleanupAfter } from "./task-retention-DxawMBrm.js";
...
function isChildlessCodexNativeSubagentTask(task) {
	if (task.runtime !== "subagent" || task.taskKind !== "codex-native") return false;
	if (task.childSessionKey?.trim()) return false;
	return [task.sourceId, task.runId].some((candidate) => candidate?.trim().startsWith(CODEX_NATIVE_SUBAGENT_RUN_ID_PREFIX));
}
//#endregion
...
function ensureSqliteStorePermissions(params) {
	mkdirSync(params.dir, {
		recursive: true,
		mode: params.dirMode
	});
...
function rebuildLegacyFlowRunsTable(db) {
	db.exec(`
...
function listTaskRegistryRecordsByOwnerKeyFromSqlite(ownerKey) {
	...
	if (!key) return [];
	const { statements } = openTaskRegistryDatabase();
	return statements.selectByOwnerKey.all(key).map(rowToTaskRecord);
}
...
function loadTaskRegistryDeliveryRuntime() {
	...
	deliveryRuntimePromise ??= import("./task-registry-delivery-runtime-D8IJXAGa.js");
...
async function maybeDeliverTaskTerminalUpdate(taskId) {
	...
	try {
		...
		if (shouldRouteParentReview || !canDeliverDirect) try {
...
async function cancelTaskById(params) {
	...
	try {
		if (task.runtime !== "cli") {
			if (!childSessionKey) {
				if (!isChildlessCodexNativeSubagentTask(task)) return {
					found: true,
					cancelled: false,
			...
			if (!childSessionKey) {} else if (task.runtime === "acp") {
