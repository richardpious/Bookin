import { S as resolveTimerTimeoutMs, _ as resolveExpiresAtMsFromDurationSeconds, h as positiveSecondsToSafeMilliseconds, l as nonNegativeSecondsToSafeMilliseconds, m as parseStrictPositiveInteger, v as resolveExpiresAtMsFromEpochSeconds } from "./number-coercion-DjYijOzx.js";
...
//#region src/llm/utils/oauth/abort.ts
...
//#endregion
//#region src/llm/utils/oauth/oauth-page.ts
...
//#endregion
//#region src/llm/utils/oauth/pkce.ts
/**
...
function resolveTokenExpiresAt(value) {
	...
	if (expiresInSeconds === void 0) return;
	const lifetimeMs = expiresInSeconds * 1e3;
	const expiresAt = Date.now() + lifetimeMs - 300 * 1e3;
	return Number.isSafeInteger(lifetimeMs) && Number.isSafeInteger(expiresAt) ? expiresAt : void 0;
}
...
async function loginAnthropic(options) {
	...
	try {
		...
		if (!code) {
			const parsed = parseAuthorizationInput(await withOAuthLoginAbort(options.onPrompt({
...
async function enableGitHubCopilotModel(token, modelId, enterpriseDomain, options = {}) {
	...
	try {
		...
		}, "model policy request", options)).ok;
	} catch {
		return false;
	}
}
...
//#endregion
//#region src/llm/utils/oauth/openai-codex.ts
...
async function loginOpenAICodex(callbacks) {
	...
	const { loginOpenAICodexOAuth } = await import("./provider-openai-codex-oauth-mnxBedEy.js");
