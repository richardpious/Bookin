import { n as getCommandPathWithRootOptions } from "./argv-htX0ehVt.js";
...
const DEFAULT_REDACT_PREFILTER_RE = /(?:KEY|TOKEN|SECRET|PASSWORD|PASSWD|AUTH|COOKIE|SIGNATURE|CARD|CVC|CVV|PAYMENT|PRIVATE KEY|[?&]pass=|security[-_]?code|securityCode|\bBearer\s+|sk-|ghp_|github_pat_|xox[baprs]-|xapp-|gsk_|AIza|ya29\.|1\/\/0|eyJ|pplx-|npm_|AKID|LTAI|hf_|r8_|\bbot\d{6,}:|\b\d{6,}:[A-Za-z0-9_-]{20,})/i;
...
function redactMatch(match, groups, options = {}) {
	...
	if (options.preserveShellReferences && (shouldPreserveShellReferenceMatch(match, token) || isEmptyShellParameterExpansionTail(token))) return match;
...
function redactText(text, patterns) {
	let next = text;
	for (const pattern of patterns) next = replacePatternBounded(next, pattern, (...args) => redactMatch(args[0], args.slice(1, -2), { preserveShellReferences: shellReferencePreservingPatterns.has(pattern) }));
	return next;
}
...
function redactAppSpecificPasswords(text) {
	return replacePatternBounded(text, APP_SPECIFIC_PASSWORD_RE, (match, token) => looksLikeAppSpecificPassword(token) ? redactMatch(match, [token]) : match);
}
...
//#endregion
export { hasNestedRepetition as _, redactSensitiveFieldValueWithConfig as a, redactToolDetail as c, resolveRedactOptions as d, readLoggingConfig as f, compileSafeRegexDetailed as g, compileSafeRegex as h, redactSensitiveFieldValue as i, redactToolPayloadText as l, compileConfigRegexes as m, isSensitiveFieldKey as n, redactSensitiveLines as o, shouldSkipMutatingLoggingConfigRead as p, redactSecrets as r, redactSensitiveText as s, getDefaultRedactPatterns as t, redactToolPayloadTextWithConfig as u, testRegexWithBoundedInput as v };
