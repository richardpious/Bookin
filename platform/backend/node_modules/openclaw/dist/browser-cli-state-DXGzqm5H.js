...
import { l as parseBooleanValue } from "./config-DwOUvgX-.js";
import "./core-api-UrlYKIvu.js";
import "./core-api-iEfXC4Xf.js";
import { o as parseBrowserPositiveIntegerValue, t as callBrowserRequest } from "./browser-cli-shared-Bqqxa2QZ.js";
...
function registerBrowserStateCommands(browser, parentOpts) {
	...
	set.command("viewport").description("Set viewport size (alias for resize)").argument("<width>", "Viewport width").argument("<height>", "Viewport height").option("--target-id <id>", "CDP target id (or unique prefix)").action(async (widthRaw, heightRaw, opts, cmd) => {
	...
	set.command("geo").description("Set geolocation (and grant permission)").option("--clear", "Clear geolocation + permissions", false).argument("[latitude]", "Latitude").argument("[longitude]", "Longitude").option("--accuracy <m>", "Accuracy in meters").option("--origin <origin>", "Origin to grant permissions for").option("--target-id <id>", "CDP target id (or unique prefix)").action(async (latitudeRaw, longitudeRaw, opts, cmd) => {
		const parent = parentOpts(cmd);
