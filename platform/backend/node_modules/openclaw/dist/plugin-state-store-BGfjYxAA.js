...
const MAX_ENTRIES_PER_PLUGIN = 5e4;
const MAX_PLUGIN_STATE_ENTRIES_PER_PLUGIN = MAX_ENTRIES_PER_PLUGIN;
...
function ensurePluginStatePermissions(pathname, env = process.env) {
...
function ensurePluginStatePermissionsBestEffort(pathname, env = process.env) {
	try {
		ensurePluginStatePermissions(pathname, env);
	} catch {}
}
function openPluginStateDatabase(operation = "open", env = process.env) {
...
function runWriteTransaction(operation, write, env = process.env) {
...
function enforcePostRegisterLimits(params) {
	...
	const pluginCount = countRow(params.store.statements.countLivePlugin.get(params.pluginId, params.now));
	if (pluginCount <= MAX_ENTRIES_PER_PLUGIN) return;
	params.store.statements.deleteOldestNamespace.run(params.pluginId, params.namespace, params.protectedKey, params.now, pluginCount - MAX_ENTRIES_PER_PLUGIN);
...
function pluginStateRegister(params) {
	try {
		...
		throw wrapPluginStateError(error, "register", "PLUGIN_STATE_WRITE_FAILED", "Failed to register plugin state entry.", resolvePluginStateSqlitePath(params.env));
	}
}
...
function pluginStateLookup(params) {
	try {
		...
		throw wrapPluginStateError(error, "lookup", "PLUGIN_STATE_READ_FAILED", "Failed to read plugin state entry.", resolvePluginStateSqlitePath(params.env));
	}
}
function pluginStateConsume(params) {
	try {
		...
		throw wrapPluginStateError(error, "consume", "PLUGIN_STATE_READ_FAILED", "Failed to consume plugin state entry.", resolvePluginStateSqlitePath(params.env));
	}
}
function pluginStateDelete(params) {
	try {
		...
		throw wrapPluginStateError(error, "delete", "PLUGIN_STATE_WRITE_FAILED", "Failed to delete plugin state entry.", resolvePluginStateSqlitePath(params.env));
	}
}
function pluginStateEntries(params) {
	try {
		...
		throw wrapPluginStateError(error, "entries", "PLUGIN_STATE_READ_FAILED", "Failed to list plugin state entries.", resolvePluginStateSqlitePath(params.env));
	}
}
function countPluginStateLiveEntries(pluginId) {
	try {
		const { statements } = openPluginStateDatabase("entries");
		return countRow(statements.countLivePlugin.get(pluginId, Date.now()));
	} catch (error) {
...
function pluginStateClear(params) {
	try {
		...
		throw wrapPluginStateError(error, "clear", "PLUGIN_STATE_WRITE_FAILED", "Failed to clear plugin state namespace.", resolvePluginStateSqlitePath(params.env));
	}
}
...
//#endregion
export { countPluginStateLiveEntries as a, closePluginStateSqliteStore as i, createPluginStateSyncKeyedStore as n, isPluginStateDatabaseOpen as o, MAX_PLUGIN_STATE_ENTRIES_PER_PLUGIN as r, sweepExpiredPluginStateEntries as s, createPluginStateKeyedStore as t };
