...
import { b as createToolStreamWrapper, s as createPlainTextToolCallCompatWrapper, t as composeProviderStreamWrappers } from "./provider-stream-shared-BktOwrlY.js";
...
function decodeHtmlEntitiesInObject(value) {
	switch (typeof value) {
		case "string": return HTML_ENTITY_RE.test(value) ? decodeHtmlEntities(value) : value;
		case "object":
			if (!value) return value;
			if (Array.isArray(value)) return value.map((entry) => decodeHtmlEntitiesInObject(entry));
			return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, decodeHtmlEntitiesInObject(entry)]));
		default: return value;
	}
}
function visitContentBlocks(value, visitor) {
	if (Array.isArray(value)) {
		for (const entry of value) visitContentBlocks(entry, visitor);
		return;
	}
	if (!value || typeof value !== "object") return;
	const block = value;
	visitor(block);
	if ("content" in block) visitContentBlocks(block.content, visitor);
}
function decodeToolCallArgumentsHtmlEntitiesInMessage(message) {
	visitContentBlocks(message, (block) => {
		if (block.type !== "toolCall" || !block.arguments || typeof block.arguments !== "object") return;
...
function transformXaiStreamEvent(value, transformMessage) {
	if (!value || typeof value !== "object") return;
	const event = value;
	...
	stream[Symbol.asyncIterator] = function() {
		...
		return {
			async next() {
				...
				if (!result.done) transformXaiStreamEvent(result.value, transformMessage);
				return result;
			},
			async return(value) {
