...
import { c as JsonFileReadError, p as tryReadJsonSync } from "./json-files-ruLzKqUC.js";
...
import { c as listApprovedPairedDeviceRoles, g as summarizeDeviceTokens, l as listDevicePairing } from "./device-pairing-C5JfsLW6.js";
...
function collectLocalDeviceAuthIssues(snapshot) {
	...
	for (const entry of Object.values(store.tokens)) {
		...
		if (!pairedToken) {
			...
			lines.push(`- Local cached ${role} device auth for ${deviceLabel} no longer has a matching active gateway token, and that role is no longer approved for this device. Reconnect with shared gateway auth to refresh local auth, or remove the stale cached ${role} auth entry.`);
			continue;
		}
		const rotateCommand = formatCliArgs([
			"openclaw",
