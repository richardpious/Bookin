...
import { f as resolvePathWithinRoot, w as pathExists } from "../../fs-safe-DYWmA8_q.js";
...
import { l as jsonResult } from "../../common-COS7PQnH.js";
...
import { n as resolveLivePluginConfigObject } from "../../plugin-config-runtime-DN8ojv-q.js";
import { t as bumpSkillsSnapshotVersion } from "../../refresh-state-BLFcka6V.js";
import "../../agent-runtime-DmnJEsR4.js";
import "../../api-CZY2G_UV.js";
...
function proposalFromReviewerJson(params) {
	const action = normalizeAction(normalizeOptionalString(params.parsed.action));
	if (!action || action === "none") return;
	const skillName = normalizeSkillName(normalizeOptionalString(params.parsed.skillName) ?? "");
	if (!skillName) return;
	const now = Date.now();
	const title = normalizeOptionalString(params.parsed.title) ?? `Skill update: ${skillName}`;
	const reason = normalizeOptionalString(params.parsed.reason) ?? "Review found reusable workflow";
	let change;
	if (action === "replace") {
		const oldText = normalizeOptionalString(params.parsed.oldText);
		const newText = normalizeOptionalString(params.parsed.newText);
		...
		const body = normalizeOptionalString(params.parsed.body);
		if (!body) return;
		if (action === "append") change = {
			kind: "append",
			section: normalizeOptionalString(params.parsed.section) ?? "Workflow",
			body,
			description: normalizeOptionalString(params.parsed.description) ?? title
		};
		else change = {
			kind: "create",
			description: normalizeOptionalString(params.parsed.description) ?? title,
...
async function reviewTranscriptForProposal(params) {
	...
	const parsed = parseReviewerJson(((await params.api.runtime.agent.runEmbeddedAgent({
		sessionId,
...
function buildProposal(params) {
	const skillName = normalizeSkillName(normalizeOptionalString(params.raw.skillName) ?? "");
	if (!skillName) throw new Error("skillName required");
	const now = Date.now();
	const title = normalizeOptionalString(params.raw.title) ?? `Skill update: ${skillName}`;
	const reason = normalizeOptionalString(params.raw.reason) ?? "Tool-created skill update";
	const body = normalizeOptionalString(params.raw.body);
	const description = normalizeOptionalString(params.raw.description) ?? title;
	let change;
	if (params.raw.oldText !== void 0 || params.raw.newText !== void 0) {
		const oldText = normalizeOptionalString(params.raw.oldText);
		const newText = normalizeOptionalString(params.raw.newText);
	...
	} else if (normalizeOptionalString(params.raw.section)) {
		if (!body) throw new Error("body required");
		change = {
			kind: "append",
			section: normalizeOptionalString(params.raw.section) ?? "Workflow",
			body,
			description
		};
	} else {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			if (action === "write_support_file") {
				const skillName = normalizeOptionalString(raw.skillName);
				const relativePath = normalizeOptionalString(raw.relativePath);
				const body = raw.body;
