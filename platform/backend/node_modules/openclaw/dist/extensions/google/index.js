...
import { u as normalizeResolvedSecretInputString } from "../../types.secrets-CuOz9KrZ.js";
...
import { t as buildGoogleGeminiCliBackend } from "../../cli-backend-CuDu6qEO.js";
import { n as registerGoogleGeminiCliProvider } from "../../gemini-cli-provider-Df-4JlMc.js";
import { c as createGoogleMusicGenerationProviderMetadata, l as createGoogleVideoGenerationProviderMetadata } from "../../generation-provider-metadata-B6QP3PO9.js";
import { t as geminiMemoryEmbeddingProviderAdapter } from "../../memory-embedding-adapter-DcZcj5On.js";
import { n as registerGoogleProvider } from "../../provider-registration-CKIUJcDQ.js";
import { t as buildGoogleSpeechProvider } from "../../speech-provider-CU86zPh9.js";
import { t as createGeminiWebSearchProvider } from "../../gemini-web-search-provider-Dn3ylf39.js";
...
function createLazyGoogleRealtimeVoiceBridge(req) {
	...
	const flushPending = (loadedBridge) => {
		...
		for (const audio of pendingAudio.splice(0)) loadedBridge.sendAudio(audio);
		for (const text of pendingUserMessages.splice(0)) loadedBridge.sendUserMessage?.(text);
	...
	return {
		...
		sendAudio: (audio) => {
			...
			if (!closed) {
				if (pendingAudio.length >= GOOGLE_REALTIME_LAZY_MAX_PENDING_AUDIO_CHUNKS) pendingAudio.shift();
		...
		sendUserMessage: (text) => {
			...
			if (!closed) pendingUserMessages.push(text);
		},
		triggerGreeting: (instructions) => {
			...
			if (!closed) pendingGreeting = instructions;
		},
