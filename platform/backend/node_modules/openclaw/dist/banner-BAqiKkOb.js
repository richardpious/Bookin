import { l as hasRootVersionAlias } from "./argv-htX0ehVt.js";
...
import { i as supportsDecorativeEmoji, n as decorativePrefix, r as stripDecorativeEmojiForTerminal, t as decorativeEmoji } from "./decorative-emoji-C7cb8Cby.js";
...
function formatCliBannerArtLines(options) {
	const width = visibleWidth(LOBSTER_ASCII_BODY[0] ?? "");
	const title = supportsDecorativeEmoji(resolveEmojiOptions(options)) ? "🦞 OPENCLAW 🦞" : "OPENCLAW";
	return [
		...LOBSTER_ASCII_BODY,
...
function formatCliBannerArt(options = {}) {
	...
	const icon = decorativeEmoji("🦞", resolveEmojiOptions(options));
	return lines.map((line) => {
		if (line.includes("OPENCLAW")) {
			if (!icon) return theme.info(centerText("OPENCLAW", visibleWidth(line)));
			return theme.muted("              ") + theme.accent(icon) + theme.info(" OPENCLAW ") + theme.accent(icon);
		}
