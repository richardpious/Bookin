...
function areBundledPluginsDisabled(env) {
	const value = normalizeOptionalString(env.OPENCLAW_DISABLE_BUNDLED_PLUGINS)?.toLowerCase();
...
function resolveBundledPluginRoot(env) {
	...
	const override = normalizeOptionalString(env.OPENCLAW_BUNDLED_PLUGINS_DIR);
	if (override) return resolveUserPath(override, env);
	return [
...
function listPersistedIndexPluginDirs(env, startOrder) {
	...
	for (const rawPlugin of index.plugins) {
		...
		const rootDir = normalizeOptionalString(rawPlugin.rootDir);
		if (!rootDir) continue;
		dirs.push({
			pluginDir: resolveUserPath(rootDir, env),
			rank: rawPlugin.origin === "bundled" ? 3 : 1,
			order: order++,
			origin: normalizeOptionalString(rawPlugin.origin)
		});
	}
	return dirs;
}
