...
import { n as getBridgeAuthForPort } from "./bridge-auth-registry-DEy2KuFQ.js";
import { W as DEFAULT_BROWSER_SNAPSHOT_TIMEOUT_MS, m as resolveBrowserRateLimitMessage } from "./cdp.helpers-WyU0BhTo.js";
import { i as resolveProfile, n as resolveBrowserConfig } from "./config-DwOUvgX-.js";
import { n as resolveBrowserControlAuth } from "./control-auth-C0mPaDIy.js";
...
function resolveBrowserFetchTimeoutMs(timeoutMs) {
	...
	return Math.max(1, Math.floor(parsed ?? 5e3));
}
...
function resolveBrowserClientTimeoutMs(opts, fallbackMs) {
	return typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs) ? Math.max(1, Math.floor(opts.timeoutMs)) : fallbackMs;
}
...
async function browserSnapshot(baseUrl, opts) {
	...
	const resolvedTimeoutMs = typeof opts.timeoutMs === "number" && Number.isFinite(opts.timeoutMs) && opts.timeoutMs > 0 ? Math.floor(opts.timeoutMs) : DEFAULT_BROWSER_SNAPSHOT_TIMEOUT_MS;
