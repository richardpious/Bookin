...
import { d as parseStrictFiniteNumber, u as parseFiniteNumber$1 } from "./number-coercion-DjYijOzx.js";
...
import { n as PROVIDER_LABELS, r as clampPercent } from "./provider-usage.shared-CHADzlkW.js";
...
async function fetchClaudeWebUsage(sessionKey, timeoutMs, fetchFn) {
	...
	const orgId = parsedOrgs.data?.[0]?.uuid?.trim();
...
async function fetchCodexUsage(token, accountId, timeoutMs, fetchFn) {
	...
	if (data.credits?.balance !== void 0 && data.credits.balance !== null) {
		const balance = typeof data.credits.balance === "number" ? data.credits.balance : parseStrictFiniteNumber(data.credits.balance) ?? 0;
