...
import { a as listThinkingLevels, l as BASE_THINKING_LEVELS } from "./thinking-Cq-IXzst.js";
import { r as getActivePluginChannelRegistryVersionFromState } from "./runtime-channel-state-CQNj7c3E.js";
...
function defineChatCommand(command) {
	...
	return {
		...
		nativeAliases: command.nativeAliases ? normalizeStringEntries(command.nativeAliases) : void 0,
...
function registerAlias(commands, key, ...aliases) {
	...
	for (const alias of command.textAliases) {
		...
		if (lowered) existing.add(lowered);
	}
...
function buildBuiltinChatCommands(params = {}) {
	...
	const listThinkingLevelChoices = (provider, model, catalog) => {
		return ["default", ...configuredThinkingLevels(provider, model, catalog).filter((level) => level !== "default")];
	};
	const commands = [
		defineChatCommand({
			key: "help",
