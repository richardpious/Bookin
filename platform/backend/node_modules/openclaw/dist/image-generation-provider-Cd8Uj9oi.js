...
import { m as mergeSsrFPolicies } from "./ssrf-_PxmDk0r.js";
import { r as fetchWithSsrFGuard } from "./fetch-guard-BhIEUTw7.js";
import { n as assertOkOrThrowHttpError, r as assertOkOrThrowProviderError } from "./provider-http-errors-BEw8ykZh.js";
import "./string-coerce-runtime-Ccm67Pxa.js";
import { r as isProviderApiKeyConfigured } from "./provider-auth-DL7jW6qn.js";
import { n as buildHostnameAllowlistPolicyFromSuffixAllowlist, u as ssrfPolicyFromDangerouslyAllowPrivateNetwork } from "./ssrf-policy-C6sqrjsE.js";
import "./ssrf-runtime-C5GYV25C.js";
import "./provider-http-65KY4eRc.js";
import { a as imageFileExtensionForMimeType, u as toImageDataUrl } from "./image-generation-DfnMntGO.js";
import { t as resolveFalHttpRequestConfig } from "./http-config-Dv7v5UXf.js";
...
function ensureFalModelPath(model, hasInputImages) {
	...
	if (trimmed.endsWith("/edit") || trimmed.endsWith(`/${DEFAULT_FAL_EDIT_SUBPATH}`) || trimmed.includes("/image-to-image/")) return trimmed;
	if (trimmed.startsWith("openai/gpt-image-") || trimmed.startsWith("fal-ai/nano-banana-")) return `${trimmed}/edit`;
...
function applyFalImageGeometry(params) {
	if (params.schema.geometry === "native_aspect_ratio") {
		...
		if (params.resolution && params.schema.referenceImages === "image_urls") params.requestBody.resolution = params.resolution;
		return;
	}
...
async function fetchImageBuffer(url, networkPolicy, maxBytes = DEFAULT_GENERATED_IMAGE_MAX_BYTES) {
	...
	try {
		...
		return {
			buffer: await readResponseWithLimit(response, maxBytes, { onOverflow: ({ maxBytes }) => /* @__PURE__ */ new Error(`fal generated image download exceeds ${maxBytes} bytes`) }),
			mimeType
		};
	} finally {
		await release();
