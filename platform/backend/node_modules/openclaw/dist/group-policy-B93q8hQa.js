...
import { _ as readReactionParams, b as readStringParam, g as readPositiveIntegerParam, i as createActionGate, l as jsonResult, m as readNonNegativeIntegerParam } from "./common-COS7PQnH.js";
...
import { a as resolveIMessageAccount } from "./accounts-CLUIDh6K.js";
import { n as IMESSAGE_ACTIONS, r as IMESSAGE_ACTION_NAMES, t as describeIMessageMessageTool } from "./message-tool-api-B1DlmQtl.js";
import "./client-7P5-Kcmt.js";
import { t as getCachedIMessagePrivateApiStatus } from "./private-api-status-hXzk6Zzf.js";
import { c as parseIMessageTarget } from "./targets-CvpxsMi3.js";
import { r as rememberIMessageReplyCache, t as findLatestIMessageEntryForChat } from "./monitor-reply-cache-B5TyaAZc.js";
import "./probe-6e69edXt.js";
//#region extensions/imessage/src/actions.ts
const loadIMessageActionsRuntime = createLazyRuntimeNamedExport(() => import("./actions.runtime-fMmre_B7.js"), "imessageActionsRuntime");
...
const imessageMessageActions = {
	...
	messageActionTargetAliases: {
		...
		sendWithEffect: { aliases: [
			"chatGuid",
			"chatIdentifier",
			"chatId"
		] },
		sendAttachment: { aliases: [
			"chatGuid",
			"chatIdentifier",
			"chatId"
		] },
	...
	handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
		...
		const assertPrivateApiEnabled = async () => {
			if (privateApiStatus?.available !== true) {
				const { probeIMessagePrivateApi } = await import("./probe-BnxTqWuq.js");
				privateApiStatus = await probeIMessagePrivateApi(cliPathForProbe, account.config.probeTimeoutMs ?? 1e4);
			}
			if (!privateApiStatus?.available) {
				log.warn(`iMessage ${action} blocked: private API bridge unavailable (accountId=${account.accountId}, cliPath=${cliPathForProbe}). Run \`imsg launch\` to re-inject the dylib, then \`openclaw channels status\` to refresh.`);
				throw new Error(`iMessage ${action} requires the imsg private API bridge. Run imsg launch, then openclaw channels status to refresh capability detection.`);
			}
		};
		const opts = {
