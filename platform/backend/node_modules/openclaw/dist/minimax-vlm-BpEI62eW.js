...
import { r as ensureGlobalUndiciEnvProxyDispatcher } from "./undici-global-dispatcher-CJimgdNq.js";
...
async function readErrorBodySnippet(res) {
	try {
		const body = res.body;
		if (!body || typeof body.getReader !== "function") return (await res.text()).slice(0, MINIMAX_VLM_ERROR_BODY_MAX_CHARS);
		...
		return new TextDecoder().decode(Buffer.concat(chunks, total)).slice(0, MINIMAX_VLM_ERROR_BODY_MAX_CHARS);
	} catch {
		return "";
	}
}
...
//#endregion
export { isMinimaxVlmProvider as n, minimaxUnderstandImage as r, isMinimaxVlmModel as t };
