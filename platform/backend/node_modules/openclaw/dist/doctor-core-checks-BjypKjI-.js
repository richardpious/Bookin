...
import { r as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-l2_ApGpt.js";
import { t as resolveGatewayAuthToken } from "./auth-token-resolution-Fs6feKvN.js";
import { n as resolveGatewayAuth } from "./auth-resolve-DdAjlmrG.js";
import "./auth-DdBhe_0j.js";
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-9aSYPxv7.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-D18esi4Z.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-CSH7gmPR.js";
import { n as disableUnavailableSkillsInConfig } from "./doctor-skills-core-cCtqEFSV.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-BU4y9Ky4.js";
import { i as registerHealthCheck } from "./health-check-registry-DxAtOKZD.js";
...
async function detectUnavailableSkillsWithRuntime(cfg) {
	return (await import("./doctor-core-checks.runtime.js")).detectUnavailableSkills(cfg);
}
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-CFeb7Yq2.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-CznCHdAe.js");
	const { MEMORY_SYSTEM_PROMPT, shouldSuggestMemorySystem } = await import("./doctor-workspace-Ctue4AbT.js");
	const notes = [];
...
async function collectRuntimeToolSchemaFindingsWithRuntime(ctx) {
	return (await import("./doctor-core-checks.runtime.js")).collectRuntimeToolSchemaFindings(ctx.cfg);
}
...
const hooksModelCheck = {
	...
	async detect(ctx) {
		...
		const { loadModelCatalog } = await import("./model-catalog-BoDbv-ea.js");
		const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } = await import("./model-selection-CzoRFYfh.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-D0QY1Km1.js");
...
const bootstrapSizeCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget } = await import("./bootstrap-budget-VfAXB1jR.js");
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-CVhFp5JY.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-BewarjqE.js");
...
const claudeCliCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { noteClaudeCliHealth } = await import("./doctor-claude-cli-C9FcSmve.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./oauth-tls-preflight-CSqMMoVz.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./doctor-cron-ChQzWlE9.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-BXxh_5LD.js");
...
const browserCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { noteChromeMcpBrowserReadiness } = await import("./doctor-browser-DifYspO7.js");
...
const workspaceStatusCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyWorkspaceDirs } = await import("./doctor-workspace-Ctue4AbT.js");
		const workspaceDir = resolveAgentWorkspaceDir(ctx.cfg, resolveDefaultAgentId(ctx.cfg));
...
//#endregion
export { createCoreHealthChecks as a, configValidationIssuesToHealthFindings as i, buildGatewayTokenSecretRefFixHint as n, registerCoreHealthChecks as o, buildGatewayTokenSecretRefUnavailableMessage as r, CORE_HEALTH_CHECKS as t };
