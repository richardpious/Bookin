...
import { t as packageNameMatchesId } from "./install-safe-path-DEiFZ_bF.js";
import { p as resolveUserPath } from "./utils-DlHt_n1Q.js";
import { a as isPrereleaseResolutionAllowed, c as validateRegistryNpmSpec, n as formatPrereleaseResolutionError, o as isPrereleaseSemverVersion, r as isExactSemverVersion, s as parseRegistryNpmSpec, t as compareOpenClawReleaseVersions } from "./npm-registry-spec-EtjA4w_n.js";
import { a as resolveDefaultPluginNpmDir, c as resolvePluginNpmProjectDir, d as validatePluginId, n as matchesExpectedPluginId, r as resolveDefaultPluginExtensionsDir, t as encodePluginInstallDirName, u as safePluginInstallFileName } from "./install-paths-P7XUTgg_.js";
import { i as parseComparableSemver, t as compareComparableSemver } from "./semver-compare-BoMHoO1q.js";
import { g as satisfiesPluginApiRange } from "./clawhub-eHNqclD3.js";
import { o as resolvePackageExtensionEntries } from "./manifest-DaiqPlf0.js";
import { i as resolvePackagePluginApiRange, r as validatePackageExtensionEntriesForInstall } from "./package-entry-resolution-BvblLBKi.js";
import { r as runCommandWithTimeout } from "./exec-DuIacDCM.js";
import { a as resolveNpmPackArchiveMetadata, n as createNpmMetadataEnv, o as resolveNpmSpecMetadata } from "./install-source-utils-oKQ78VaD.js";
import { n as createSafeNpmInstallEnv, t as createSafeNpmInstallArgs } from "./safe-package-install-Ct280sXs.js";
...
import { i as relinkOpenClawPeerDependenciesInManagedNpmRoot, r as linkOpenClawPeerDependencies } from "./plugin-peer-link-Ckh-dj0J.js";
import { t as resolveNpmIntegrityDriftWithDefaultMessage } from "./npm-integrity-DsZIWZKw.js";
import { n as installedPackageNeedsOpenClawPeerLinkRepair } from "./package-update-utils-CL0UqVO2.js";
...
async function resolveLatestCompatibleNpmResolution(params) {
	if (!shouldResolveLatestCompatibleNpmVersion(params.parsedSpec) || !params.currentResolution.version) return null;
	...
	const candidates = versions.filter((version) => !isPrereleaseSemverVersion(version)).filter((version) => compareNpmSemver(version, currentVersion) < 0).toSorted(compareNpmSemver).toReversed();
...
async function installPluginFromManagedNpmRoot(params) {
	...
	const rollbackFailedManagedNpmInstall = async (error) => {
	...
	if (!initialPeerSync.ok) return await rollbackFailedManagedNpmInstall(initialPeerSync.error);
	...
	if (install.code !== 0 && isNpmAliasOverrideComparatorError(install)) {
		...
		if (!aliasRetryPeerSync.ok) return await rollbackFailedManagedNpmInstall(aliasRetryPeerSync.error);
	...
	for (let peerSyncPass = 0; peerSyncPass < 10; peerSyncPass += 1) {
		...
		if (!peerSync.ok) return await rollbackFailedManagedNpmInstall(peerSync.error);
		if (!peerSync.changed) {
