...
import { n as PLUGIN_MANIFEST_FILENAME, r as getPackageManifestMetadata } from "./manifest-DaiqPlf0.js";
import { t as loadPluginManifestRegistry } from "./manifest-registry-BSpfEroo.js";
import { a as resolveManifestContractPluginIds } from "./plugin-registry-ChAglW1O.js";
import { l as resolveLoaderPackageRoot } from "./sdk-alias-BPp2p_OO.js";
import { t as loadBundledPluginPublicArtifactModuleSync } from "./public-surface-loader-DhFvfS89.js";
import { i as resolveBundledPluginScanDir, r as normalizeBundledPluginStringList } from "./bundled-plugin-metadata-DaxImrwh.js";
import { t as loadBundledCapabilityRuntimeRegistry } from "./bundled-capability-runtime-BVk8sYtT.js";
...
const VITEST_CONTRACT_PLUGIN_IDS = {
	...
	transcriptSourceProviders: BUNDLED_PLUGIN_CONTRACT_SNAPSHOTS.filter((entry) => entry.transcriptSourceProviderIds.length > 0).map((entry) => entry.pluginId),
...
function loadVitestTranscriptsSourceProviderContractRegistry() {
	return loadVitestCapabilityContractEntries({
		contract: "transcriptSourceProviders",
		pluginSdkResolution: "src",
		pickEntries: (registry) => registry.transcriptSourceProviders.map((entry) => ({
			pluginId: entry.pluginId,
			provider: entry.provider
		}))
	});
...
function resolveBundledManifestPluginIdsForContract(contract) {
	return uniqueStrings(resolveBundledManifestContracts().filter((entry) => {
		switch (contract) {
			case "embeddingProviders": return entry.embeddingProviderIds.length > 0;
			...
			case "transcriptSourceProviders": return entry.transcriptSourceProviderIds.length > 0;
...
function loadTranscriptsSourceProviderContractRegistry() {
	return process.env.VITEST ? loadVitestTranscriptsSourceProviderContractRegistry() : loadBundledCapabilityRuntimeRegistry({
