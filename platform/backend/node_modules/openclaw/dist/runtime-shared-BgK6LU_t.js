...
import { i as isFailoverError, r as describeFailoverError } from "./failover-error-CqeQIfYV.js";
//#region src/media-generation/capability-model-ref.ts
...
function findCapabilityProviderById(params) {
	...
	return params.providers.find((provider) => {
		if (normalizeProviderForMatch(provider.id, params.normalizeProviderId) === selectedProvider) return true;
		return (provider.aliases ?? []).some((alias) => normalizeProviderForMatch(alias, params.normalizeProviderId) === selectedProvider);
	});
}
...
//#endregion
export { normalizeDurationToClosestMax as a, resolveClosestAspectRatio as c, resolveMediaProviderDefaultTimeoutMs as d, resolveMediaProviderRequestTimeoutMs as f, resolveCapabilityModelRefForProviders as h, hasMediaNormalizationEntry as i, resolveClosestResolution as l, findCapabilityProviderById as m, buildNoCapabilityModelConfiguredMessage as n, recordCapabilityCandidateFailure as o, throwCapabilityGenerationFailure as p, deriveAspectRatioFromSize as r, resolveCapabilityModelCandidates as s, buildMediaGenerationNormalizationMetadata as t, resolveClosestSize as u };
