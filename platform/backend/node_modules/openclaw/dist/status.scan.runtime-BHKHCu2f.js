...
import { n as sha256HexPrefix } from "./redact-identifier-BI6BTlLC.js";
import { p as listExplicitConfiguredChannelIdsForConfig } from "./channel-plugin-ids-Di5PV7SC.js";
import { t as resolveMissingOfficialExternalChannelPluginRepairHint } from "./official-external-plugin-repair-hints-D-HUUEXC.js";
import { i as resolveChannelDefaultAccountId } from "./helpers-jaysAyiD.js";
import { r as resolveReadOnlyChannelPluginsForConfig } from "./read-only-cdWuNzoD.js";
import { n as formatTimeAgo } from "./format-relative-Bjc3l98W.js";
import { t as hasConfiguredUnavailableCredentialStatus } from "./account-snapshot-fields-CHPqOzoU.js";
import { i as formatChannelAllowFrom, n as resolveInspectedChannelAccount, r as buildChannelAccountSnapshot } from "./account-inspection-2RinhQs7.js";
import { t as formatChannelStatusState } from "./status-state-Dua3XQiC.js";
import { n as hasRuntimeCredentialAvailable, r as markConfiguredUnavailableCredentialStatusesAvailable, t as getRuntimeChannelAccounts } from "./read-model-VSfrBDRf.js";
import { t as collectChannelStatusIssues } from "./channels-status-issues-KFMSbCmo.js";
import "./format-BZ_DbUf3.js";
...
const buildAccountNotes = (params) => {
	...
	else if (params.credentialResolutionSkipped && hasConfiguredUnavailableCredentialStatus(entry.account)) notes.push("credential not checked");
...
async function buildChannelsTable(cfg, opts) {
	...
	for (const plugin of readOnlyPlugins.plugins) {
		...
		const unavailableConfiguredAccounts = enabledAccounts.filter((a) => hasConfiguredUnavailableCredentialStatus(a.account) && !credentialResolutionSkipped && !hasRuntimeCredentialAvailable({
			liveAccounts,
			accountId: a.accountId
		}));
		const accountsForTokenSummary = accounts.map((entry) => hasConfiguredUnavailableCredentialStatus(entry.account) && (credentialResolutionSkipped || hasRuntimeCredentialAvailable({
			liveAccounts,
		...
		if (configuredAccounts.length > 0) details.push({
			...
			rows: configuredAccounts.map((entry) => {
				...
				const credentialUnknown = credentialResolutionSkipped && hasConfiguredUnavailableCredentialStatus(entry.account);
				...
				return {
					...
					Status: entry.enabled && (!hasConfiguredUnavailableCredentialStatus(entry.account) || liveCredentialAvailable) ? "OK" : credentialUnknown ? "UNKNOWN" : "WARN",
					Notes: notes.join(" · ")
				};
			})
		});
	...
	const missingCandidateChannelIds = [...new Set([
	...
	const explicitConfiguredChannelIds = new Set([...listExplicitConfiguredChannelIdsForConfig(sourceConfig), ...listExplicitConfiguredChannelIdsForConfig(cfg)]);
