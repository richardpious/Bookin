...
import { d as resolveAgentIdFromSessionKey, h as scopeLegacySessionKeyToAgent, l as normalizeAgentId, o as classifySessionKeyShape, s as isUnscopedSessionKeySentinel } from "./session-key-CJf5_zWs.js";
import { c as resolveDefaultAgentId, n as listAgentIds } from "./agent-scope-config-CaXAl7-j.js";
...
import { d as isGatewayTransportError, f as randomIdempotencyKey, i as callGateway } from "./call-BuKoI5AV.js";
...
const defaultAgentSessionModuleLoader = () => import("./session-DwBI0nSm.js");
let agentSessionModuleLoader = defaultAgentSessionModuleLoader;
function loadEmbeddedAgentCommand() {
	embeddedAgentCommandPromise ??= import("./agent-B2EUUdmj.js").then((module) => module.agentCommand);
...
function getGatewayDispatchConfig() {
...
function isControlCommandThatMustNotFallback(opts) {
...
function normalizeSessionKeyOptsForDispatch(opts) {
	...
	const cfg = isLegacySessionKey && (agentIdRaw || shouldScopeDefaultAgentKey) ? opts.local === true ? getRuntimeConfig() : getGatewayDispatchConfig() : void 0;
...
function resolveAgentCliProcessLike(deps) {
	if (!deps || !Object.prototype.hasOwnProperty.call(deps, "process")) return process;
...
async function resolveAgentIdForGatewayTimeoutFallback(opts) {
	...
	if (isUnscopedSessionKeySentinel(explicitSessionKey)) return resolveDefaultAgentId(getGatewayDispatchConfig());
	const agentIdRaw = opts.agent?.trim();
...
async function agentViaGatewayCommand(opts, runtime, signalBridge) {
	...
	const explicitSessionKey = opts.sessionKey?.trim();
	if (!body) throw new Error(`Missing message. Use ${formatCliCommand("openclaw agent --message \"...\" --agent <id>")} or pass --to/--session-key/--session-id for an existing conversation.`);
	...
	const gatewayIdentity = Boolean(modelOverride) ? {
	...
	try {
		...
		try {
			...
			if (isGatewayAgentTimeoutError(err)) {
				if (isControlCommandThatMustNotFallback(dispatchOpts)) throw err;
