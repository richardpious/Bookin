...
function listBundledChatChannelEntries() {
	return GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA.filter((entry) => entry.configurable !== false).map((entry) => ({
...
let runtimeBundledChatChannelEntries = null;
...
function listRuntimeBundledChatChannelEntries() {
...
function normalizeRuntimeBundledChatChannelId(normalized) {
	for (const entry of listRuntimeBundledChatChannelEntries()) if (entry.id === normalized || entry.aliases.includes(normalized)) return entry.id;
	return null;
}
...
//#endregion
export { GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA as a, listBundledChannelCatalogEntries as i, CHAT_CHANNEL_ORDER as n, normalizeChatChannelId as r, CHANNEL_IDS as t };
