...
import { r as normalizeChatChannelId } from "./ids-BxC_RKqP.js";
import "./defaults-mDjiWzE5.js";
import { i as parseModelRef } from "./model-selection-normalize-DUxQN8bc.js";
import { n as pickSandboxToolPolicy } from "./sandbox-tool-policy-CAhGEmA6.js";
import { h as resolveToolProfilePolicy, l as mergeAlsoAllowPolicy } from "./tool-policy-CY4w5sXw.js";
import { n as isToolAllowedByPolicyName, t as isToolAllowedByPolicies } from "./tool-policy-match-F9QA397_.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-DDaX9To-.js"));
...
function hasExplicitChannelPluginBlockerConfig(cfg) {
	...
	return Object.values(entries).some((entry) => isRecord(entry) && "enabled" in entry && entry.enabled === false);
}
...
function collectProfileConfiguredToolSectionScopeWarnings(params) {
	...
	const uncoveredEntries = configuredEntries.map((entry) => {
		return {
...
function resolveInheritedProviderPolicyForPreview(inheritedByProvider, providerKey) {
	...
	const policy = resolveProviderToolPolicy({
		...
		modelId: slashIndex > 0 ? normalized.slice(slashIndex + 1) : ""
	});
	return policy && isRecord(policy) ? policy : void 0;
}
function resolveProviderPolicyEntryForPreview(params) {
	if (!params.byProvider || !params.modelProvider) return;
	const lookup = /* @__PURE__ */ new Map();
	for (const [key, value] of Object.entries(params.byProvider)) {
		const policy = isRecord(value) ? value : void 0;
		if (!policy) continue;
		const normalized = normalizeProviderPolicyKey(key);
		if (!normalized) continue;
		const canonical = isCanonicalProviderPolicyKey(key);
...
function collectProfileConfiguredToolSectionWarnings(cfg) {
	...
	listAgentRecords(cfg).forEach((agent, index) => {
		...
		const modelRef = resolvePrimaryModelRef(cfg, (agentId ? resolveAgentConfig(cfg, agentId) : void 0)?.model);
...
async function collectDoctorPreviewNotes(params) {
	...
	const { collectActiveToolSchemaProjectionWarnings } = await import("./active-tool-schema-warnings-DrYtEX5f.js");
	warnings.push(...collectActiveToolSchemaProjectionWarnings({
		cfg: params.cfg,
		env
	}));
	const channelPluginRuntime = hasChannelConfig && hasExplicitChannelPluginBlockerConfig(params.cfg) ? await import("./channel-plugin-blockers-a-80SUTI.js") : void 0;
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-CMrRtN--.js");
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-DUUQU2JP.js");
	...
	if (hasPluginConfig) {
		const { collectCodexRouteWarnings } = await import("./codex-route-warnings-gSzeVtIX.js");
		warnings.push(...collectCodexRouteWarnings({
			cfg: params.cfg,
			env
		}));
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-zxwILzLo.js");
	...
	if (hasSubagentAllowlistConfig(params.cfg)) {
		const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } = await import("./stale-subagent-allowlist-CSmttbeP.js");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-DxhhQxO0.js");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-Czl8_LFT.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-B9B_JhhO.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-Xrb4Fuim.js");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-Dq25niX9.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-BjP_Gwmr.js");
