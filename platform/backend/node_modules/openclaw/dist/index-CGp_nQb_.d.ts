import { _ as Usage, a as CompleteSimpleFn, c as Message, d as StreamFn$1, f as TextContent, g as Transport, h as ToolResultMessage, i as AssistantMessageEvent, l as Model, m as Tool, o as EventStream, p as ThinkingBudgets, r as AssistantMessage, s as ImageContent, u as SimpleStreamOptions } from "./validation-Cys4yUnY.js";
import { Static, TSchema } from "typebox";

//#region packages/agent-core/src/runtime-deps.d.ts
interface AgentCoreRuntimeDeps {
  streamSimple: StreamFn$1;
...
declare function resolveAgentCoreStreamFn(runtime: AgentCoreStreamRuntimeDeps | undefined, streamFn?: StreamFn$1): StreamFn$1;
...
type AgentToolCall = Extract<AssistantMessage["content"][number], {
  type: "toolCall";
}>;
/**
 ...
 * ```
 */
interface CustomAgentMessages extends Record<never, never> {}
/**
 ...
 */
declare function agentLoop(prompts: AgentMessage[], context: AgentContext, config: AgentLoopConfig, signal?: AbortSignal, streamFn?: StreamFn, runtime?: AgentCoreStreamRuntimeDeps): EventStream<AgentEvent, AgentMessage[]>;
/**
 ...
 */
declare function agentLoopContinue(context: AgentContext, config: AgentLoopConfig, signal?: AbortSignal, streamFn?: StreamFn, runtime?: AgentCoreStreamRuntimeDeps): EventStream<AgentEvent, AgentMessage[]>;
declare function runAgentLoop(prompts: AgentMessage[], context: AgentContext, config: AgentLoopConfig, emit: AgentEventSink, signal?: AbortSignal, streamFn?: StreamFn, runtime?: AgentCoreStreamRuntimeDeps): Promise<AgentMessage[]>;
declare function runAgentLoopContinue(context: AgentContext, config: AgentLoopConfig, emit: AgentEventSink, signal?: AbortSignal, streamFn?: StreamFn, runtime?: AgentCoreStreamRuntimeDeps): Promise<AgentMessage[]>;
//#endregion
...
declare class AgentHarness<TSkill extends Skill = Skill, TPromptTemplate extends PromptTemplate = PromptTemplate, TTool extends AgentTool = AgentTool> {
...
declare function getOrThrow<TValue, TError>(result: Result<TValue, TError>): TValue;
/** Return the success value or `undefined`. Only object values are allowed to avoid truthiness bugs with primitives. */
declare function getOrUndefined<TValue extends object, TError>(result: Result<TValue, TError>): TValue | undefined;
...
interface SessionRepo<TMetadata extends SessionMetadata = SessionMetadata, TCreateOptions extends SessionCreateOptions = SessionCreateOptions, TListOptions = void> {
...
interface JsonlSessionListOptions {
  cwd?: string;
}
interface JsonlSessionRepoApi extends SessionRepo<JsonlSessionMetadata, JsonlSessionCreateOptions, JsonlSessionListOptions> {}
...
interface AgentHarnessPromptOptions {
  images?: ImageContent[];
}
...
//#endregion
//#region packages/agent-core/src/harness/prompt-templates.d.ts
type PromptTemplateDiagnosticCode = "file_info_failed" | "list_failed" | "read_failed" | "parse_failed";
/** Warning produced while loading prompt templates. */
interface PromptTemplateDiagnostic {
 ...
 */
declare function loadPromptTemplates(env: ExecutionEnv, paths: string | string[]): Promise<{
 ...
 */
declare function loadSourcedPromptTemplates<TSource, TPromptTemplate extends PromptTemplate = PromptTemplate>(env: ExecutionEnv, inputs: Array<{
  path: string;
  source: TSource;
...
type SkillDiagnosticCode = "file_info_failed" | "list_failed" | "read_failed" | "parse_failed" | "invalid_metadata";
 ...
 */
declare function loadSkills(env: ExecutionEnv, dirs: string | string[]): Promise<{
  skills: Skill[];
 ...
 */
declare function loadSourcedSkills<TSource, TSkill extends Skill = Skill>(env: ExecutionEnv, inputs: Array<{
  path: string;
  source: TSource;
...
//#endregion
//#region packages/agent-core/src/harness/system-prompt.d.ts
declare function formatSkillsForSystemPrompt(skills: Skill[]): string;
//#endregion
//#region packages/agent-core/src/harness/session/jsonl-repo.d.ts
...
declare class JsonlSessionRepo implements JsonlSessionRepoApi {
  private readonly fs;
...
declare abstract class BaseSessionStorage<TMetadata extends SessionMetadata = SessionMetadata> implements SessionStorage<TMetadata> {
  ...
  private leafId;
  protected constructor(metadata: TMetadata, entries: SessionTreeEntry[], leafId?: string | null);
  getMetadata(): Promise<TMetadata>;
  getLeafId(): Promise<string | null>;
...
//#endregion
//#region packages/agent-core/src/harness/session/memory-repo.d.ts
declare class InMemorySessionRepo implements SessionRepo<SessionMetadata, {
  id?: string;
}> {
  private sessions;
  create(options?: {
...
//#endregion
//#region packages/agent-core/src/harness/session/repo-utils.d.ts
declare function createSessionId(): string;
declare function createTimestamp(): string;
declare function toSession<TMetadata extends SessionMetadata>(storage: SessionStorage<TMetadata>): Session<TMetadata>;
declare function getFileSystemResultOrThrow<TValue>(result: Result<TValue, FileError>, message: string): TValue;
declare function getEntriesToFork(storage: SessionStorage, options: {
  entryId?: string;
  position?: "before" | "at";
...
declare function generateSummary(currentMessages: AgentMessage[], model: Model, reserveTokens: number, apiKey: string | undefined, headers?: Record<string, string>, signal?: AbortSignal, customInstructions?: string, previousSummary?: string, thinkingLevel?: ThinkingLevel, streamFn?: StreamFn$1, runtime?: AgentCoreCompletionRuntimeDeps): Promise<Result<string, CompactionError>>;
...
declare function compact(preparation: CompactionPreparation, model: Model, apiKey: string | undefined, headers?: Record<string, string>, customInstructions?: string, signal?: AbortSignal, thinkingLevel?: ThinkingLevel, streamFn?: StreamFn$1, runtime?: AgentCoreCompletionRuntimeDeps): Promise<Result<CompactionResult, CompactionError>>;
//#endregion
//#region packages/agent-core/src/harness/utils/shell-output.d.ts
...
declare function sanitizeBinaryOutput(str: string): string;
declare function executeShellWithCapture(env: ExecutionEnv, command: string, options?: ShellCaptureOptions): Promise<Result<ShellCaptureResult, ExecutionError>>;
//#endregion
...
//#endregion
export { SkillDiagnosticCode as $, ThinkingLevelSelectEvent as $n, CompactionError as $t, BranchPathEntry as A, Result as An, AgentToolProgress as Ar, AgentHarnessError as At, uuidv7 as B, SessionErrorCode as Bn, ThinkingLevel as Br, AgentHarnessStreamOptionsPatch as Bt, estimateTokens as C, ModelChangeEntry as Cn, AgentEvent as Cr, KillProcessTreeOptions as Ct, getLastAssistantUsage as D, PromptTemplate as Dn, AgentState as Dr, AbortEvent as Dt, generateSummary as E, PendingSessionWrite as En, AgentMessage as Er, NodeExecutionEnv as Et, collectEntriesForBranchSummary as F, SessionBeforeTreeResult as Fn, CustomAgentMessages as Fr, AgentHarnessOwnEvent as Ft, toSession as G, SessionStorage as Gn, resolveAgentCoreCompleteFn as Gr, BeforeProviderRequestEvent as Gt, createTimestamp as H, SessionInfoEntry as Hn, AgentCoreCompletionRuntimeDeps as Hr, BeforeAgentStartResult as Ht, collectEntriesForBranchSummaryFromBranches as I, SessionCompactEvent as In, PrepareNextTurnContext as Ir, AgentHarnessPhase as It, JsonlSessionStorage as J, SessionTreeEvent as Jn, BranchSummaryError as Jt, InMemorySessionStorage as K, SessionTreeEntry as Kn, resolveAgentCoreStreamFn as Kr, BeforeProviderRequestResult as Kt, generateBranchSummary as L, SessionContext as Ln, QueueMode as Lr, AgentHarnessPromptOptions as Lt, BranchSummaryDetails as M, SessionBeforeCompactEvent as Mn, AgentToolUpdateCallback as Mr, AgentHarnessEvent as Mt, CollectBranchPathEntriesResult as N, SessionBeforeCompactResult as Nn, BeforeToolCallContext as Nr, AgentHarnessEventResultMap as Nt, prepareCompaction as O, QueueUpdateEvent as On, AgentTool as Or, AbortResult as Ot, CollectEntriesResult as P, SessionBeforeTreeEvent as Pn, BeforeToolCallResult as Pr, AgentHarnessOptions as Pt, SkillDiagnostic as Q, ThinkingLevelChangeEntry as Qn, CompactionEntry as Qt, prepareBranchEntries as R, SessionCreateOptions as Rn, ShouldStopAfterTurnContext as Rr, AgentHarnessResources as Rt, estimateContextTokens as S, MessageEntry as Sn, AgentContext as Sr, createCustomMessage as St, findTurnStartIndex as T, NavigateTreeResult as Tn, AgentLoopTurnUpdate as Tr, signalProcessTree as Tt, getEntriesToFork as U, SessionMetadata as Un, AgentCoreRuntimeDeps as Ur, BeforeProviderPayloadEvent as Ut, createSessionId as V, SessionForkOptions as Vn, ToolExecutionMode as Vr, BeforeAgentStartEvent as Vt, getFileSystemResultOrThrow as W, SessionRepo as Wn, AgentCoreStreamRuntimeDeps as Wr, BeforeProviderPayloadResult as Wt, JsonlSessionRepo as X, Shell as Xn, BranchSummaryResult as Xt, loadJsonlSessionMetadata as Y, SettledEvent as Yn, BranchSummaryErrorCode as Yt, formatSkillsForSystemPrompt as Z, Skill as Zn, CompactResult as Zt, CompactionSettings as _, JsonlSessionListOptions as _n, runAgentLoopContinue as _r, CustomMessage as _t, TruncationResult as a, ExecutionEnv as an, err as ar, formatPromptTemplateInvocation as at, calculateContextTokens as b, LabelEntry as bn, AfterToolCallContext as br, createBranchSummaryMessage as bt, truncateLine as c, ExecutionErrorCode as cn, ok as cr, parseCommandArgs as ct, ShellCaptureResult as d, FileInfo as dn, Session as dr, BRANCH_SUMMARY_SUFFIX as dt, CompactionErrorCode as en, ToolCallEvent as er, formatSkillInvocation as et, executeShellWithCapture as f, FileKind as fn, buildSessionContext as fr, BashExecutionMessage as ft, CompactionResult as g, JsonlSessionCreateOptions as gn, runAgentLoop as gr, CompactionSummaryMessage as gt, CompactionPreparation as h, GenerateBranchSummaryOptions$1 as hn, agentLoopContinue as hr, COMPACTION_SUMMARY_SUFFIX as ht, TruncationOptions as i, CustomMessageEntry as in, TreePreparation as ir, PromptTemplateDiagnosticCode as it, BranchPreparation as j, SavePointEvent as jn, AgentToolResult as jr, AgentHarnessErrorCode as jt, shouldCompact as k, ResourcesUpdateEvent as kn, AgentToolCall as kr, AfterProviderResponseEvent as kt, truncateTail as l, FileError as ln, toError as lr, substituteArgs as lt, CompactionDetails as m, FileSystem as mn, agentLoop as mr, COMPACTION_SUMMARY_PREFIX as mt, DEFAULT_MAX_LINES as n, ContextResult as nn, ToolResultEvent as nr, loadSourcedSkills as nt, formatSize as o, ExecutionEnvExecOptions as on, getOrThrow as or, loadPromptTemplates as ot, sanitizeBinaryOutput as p, FileOperations$1 as pn, AgentEventSink as pr, BranchSummaryMessage as pt, InMemorySessionRepo as q, SessionTreeEntryBase as qn, BranchSummaryEntry as qt, GREP_MAX_LINE_LENGTH as r, CustomEntry as rn, ToolResultPatch as rr, PromptTemplateDiagnostic as rt, truncateHead as s, ExecutionError as sn, getOrUndefined as sr, loadSourcedPromptTemplates as st, DEFAULT_MAX_BYTES as t, ContextEvent as tn, ToolCallResult as tr, loadSkills as tt, ShellCaptureOptions as u, FileErrorCode as un, AgentHarness as ur, BRANCH_SUMMARY_PREFIX as ut, ContextUsageEstimate as v, JsonlSessionMetadata as vn, Agent as vr, bashExecutionToText as vt, findCutPoint as w, ModelSelectEvent as wn, AgentLoopConfig as wr, killProcessTree as wt, compact as x, LeafEntry as xn, AfterToolCallResult as xr, createCompactionSummaryMessage as xt, DEFAULT_COMPACTION_SETTINGS as y, JsonlSessionRepoApi as yn, AgentOptions as yr, convertToLlm as yt, serializeConversation as z, SessionError as zn, StreamFn as zr, AgentHarnessStreamOptions as zt };