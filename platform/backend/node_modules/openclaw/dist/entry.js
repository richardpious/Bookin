#!/usr/bin/env node
import { i as normalizeEnv, t as isTruthyEnvValue } from "./env-B-C-u8eW.js";
import { c as hasFlag, d as isRootHelpInvocation, f as isRootVersionInvocation, n as getCommandPathWithRootOptions, r as getCommandPositionalsWithRootOptions } from "./argv-htX0ehVt.js";
import { t as isMainModule } from "./is-main-DoKHeRG7.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-DOvZlpGg.js";
import { a as resolveCliContainerTarget, i as parseCliContainerArgs, n as parseCliProfileArgs, t as applyCliProfileEnv } from "./profile-ClaYNNIL.js";
...
const COMPILE_CACHE_RESPAWN_SIGNAL_HARD_EXIT_GRACE_MS = 1e3;
...
const CLI_RESPAWN_SIGNAL_HARD_EXIT_GRACE_MS = 1e3;
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-CH9gLQAQ.js"), import("./git-commit-BrEQ7Hbe.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		const liveRootHelpOptions = await (deps.loadRootHelpRenderOptionsForConfigSensitivePlugins ?? (await import("./root-help-live-config-DFzd7blV.js")).loadRootHelpRenderOptionsForConfigSensitivePlugins)(deps.env);
		...
		await (deps.outputRootHelp ?? (await import("./root-help-le8sGUbT.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
		handleError(error);
		return true;
...
async function tryHandlePrecomputedCommandHelpFastPath(argv, deps = {}) {
	...
	try {
		if (commandName === "nodes") {
			if (await (deps.loadRootHelpRenderOptionsForConfigSensitivePlugins ?? (await import("./root-help-live-config-DFzd7blV.js")).loadRootHelpRenderOptionsForConfigSensitivePlugins)(env)) return false;
		}
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-CEyP7oO7.js");
