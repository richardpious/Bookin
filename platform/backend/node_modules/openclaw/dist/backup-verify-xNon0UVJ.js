...
function verifyHardlinkTargetsAgainstArchiveRoot(hardlinkTargets, archiveRoot, entries) {
	...
	for (const target of hardlinkTargets) {
		if (!isArchivePathWithin(target.normalized, normalizedRoot)) throw new Error(`Archive hardlink target is outside the declared archive root: ${target.entryPath} -> ${target.normalized}`);
		if (!entries.has(target.normalized)) throw new Error(`Archive hardlink target is missing from archive entries: ${target.entryPath} -> ${target.normalized}`);
	}
}
