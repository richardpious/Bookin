...
import { n as resolveCliName } from "./cli-name-C-K1n6ar.js";
...
import { p as resolveUserPath, u as pathExists } from "./utils-DlHt_n1Q.js";
import { n as buildGatewayInstallPlan, r as gatewayInstallErrorHint } from "./auth-install-policy-B1jG2vqC.js";
import { a as isSystemdUserServiceAvailable } from "./systemd-_EUbsUBb.js";
import { s as resolveDefaultAgentDir } from "./agent-scope-config-CaXAl7-j.js";
...
import { n as DEFAULT_BOOTSTRAP_FILENAME } from "./workspace-BF52cjJK.js";
import { t as describeCodexNativeWebSearch } from "./codex-native-web-search.shared-DTOeJgCP.js";
import { i as hasAuthProfileForProvider } from "./model-config.helpers-DKG6J5Qj.js";
import { n as listConfiguredWebSearchProviders } from "./runtime-B-kpt-pU.js";
import { a as installCompletion, i as formatCompletionReloadCommand, l as resolveCompletionProfilePath } from "./completion-runtime-k7R23upj.js";
...
import { n as openUrl, t as detectBrowserOpenSupport } from "./browser-open-CSqiOr-7.js";
import { i as formatControlUiSshHint, m as waitForGatewayReachable, u as probeGatewayReachable } from "./onboard-helpers-BelfoDhi.js";
...
function loadOnboardSearchModule() {
	onboardSearchModulePromise ??= import("./onboard-search-CM1IZMVV.js");
...
async function finalizeSetupWizard(options) {
	...
	if (process.platform === "linux" && systemdAvailable) {
		const { ensureSystemdUserLingerInteractive } = await import("./systemd-linger-NHqn92CP.js");
		...
		if (hatchChoice === "tui") {
			...
			if ((await detectBrowserOpenSupport()).ok) {
				...
				if (!controlUiOpened) controlUiOpenHint = formatControlUiSshHint({
					port: settings.port,
					basePath: controlUiBasePath,
					token: settings.authMode === "token" && !suppressGatewayTokenOutput ? settings.gatewayToken : void 0
				});
			...
			await prompter.note([
				...
				controlUiOpened ? t("wizard.finalize.dashboardOpened") : t("wizard.finalize.dashboardCopyPaste"),
				controlUiOpenHint
	...
	if (!opts.skipUi && gatewayProbe.ok && settings.authMode === "token" && Boolean(settings.gatewayToken) && !suppressGatewayTokenOutput && hatchChoice === null) {
		if ((await detectBrowserOpenSupport()).ok) {
			controlUiOpened = await openUrl(authedUrl);
			if (!controlUiOpened) controlUiOpenHint = formatControlUiSshHint({
				port: settings.port,
