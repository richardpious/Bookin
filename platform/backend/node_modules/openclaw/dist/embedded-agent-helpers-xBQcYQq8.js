...
import "./sanitize-user-facing-text-CY8fNjm7.js";
import { v as isReasoningConstraintErrorMessage } from "./errors-t09uTJ5G.js";
import { a as sanitizeToolCallIdsForCloudCodeAssist, n as extractToolResultId, t as extractToolCallsFromAssistant } from "./tool-call-id-B9whgasj.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-jxvaHCzc.js";
import { n as sanitizeGoogleAssistantFirstOrdering } from "./google-models-8_1zcSQD.js";
...
function resolveBootstrapMaxChars(cfg, agentId) {
	const raw = cfg && agentId ? resolveAgentConfig(cfg, agentId)?.bootstrapMaxChars ?? cfg.agents?.defaults?.bootstrapMaxChars : cfg?.agents?.defaults?.bootstrapMaxChars;
	if (typeof raw === "number" && Number.isFinite(raw) && raw > 0) return Math.floor(raw);
	return DEFAULT_BOOTSTRAP_MAX_CHARS;
}
function resolveBootstrapTotalMaxChars(cfg, agentId) {
	const raw = cfg && agentId ? resolveAgentConfig(cfg, agentId)?.bootstrapTotalMaxChars ?? cfg.agents?.defaults?.bootstrapTotalMaxChars : cfg?.agents?.defaults?.bootstrapTotalMaxChars;
	if (typeof raw === "number" && Number.isFinite(raw) && raw > 0) return Math.floor(raw);
...
//#endregion
export { stripThoughtSignatures as S, ensureSessionHeader as _, isMessagingToolDuplicate as a, resolveBootstrapTotalMaxChars as b, sanitizeSessionMessagesImages as c, normalizeOpenAIResponsesToolCallIds as d, isGoogleModelApi as f, buildBootstrapContextFiles as g, DEFAULT_BOOTSTRAP_TOTAL_MAX_CHARS as h, pickFallbackThinkingLevel as i, downgradeOpenAIFunctionCallReasoningPairs as l, DEFAULT_BOOTSTRAP_PROMPT_TRUNCATION_WARNING_MODE as m, validateAnthropicTurns as n, isMessagingToolDuplicateNormalized as o, DEFAULT_BOOTSTRAP_MAX_CHARS as p, validateGeminiTurns as r, normalizeTextForComparison as s, mergeConsecutiveUserTurns as t, downgradeOpenAIReasoningBlocks as u, resolveBootstrapMaxChars as v, sanitizeGoogleTurnOrdering as x, resolveBootstrapPromptTruncationWarningMode as y };
