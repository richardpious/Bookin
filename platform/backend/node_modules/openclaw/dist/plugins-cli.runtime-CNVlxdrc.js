...
import { h as shortenHomeInString } from "./utils-DlHt_n1Q.js";
...
import { a as formatMissingPluginMessage } from "./error-format-ZFU1PhmU.js";
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-dgUz3lL6.js";
...
async function runPluginsEnableCommand(id) {
	...
	const { enablePluginInConfig } = await import("./enable-DD_YOhg2.js");
	const { normalizePluginId } = await import("./config-state-BEMNlR9A.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-C-PN3sys.js");
	const { applySlotSelectionForPlugin, logSlotWarnings } = await import("./plugins-command-helpers-B_YfLm_4.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-BX48_p33.js");
...
async function runPluginsDisableCommand(id) {
	...
	const { normalizePluginId } = await import("./config-state-BEMNlR9A.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-C-PN3sys.js");
	const { setPluginEnabledInConfig } = await import("./plugins-config-BGiO77CM.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-BX48_p33.js");
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-CCknZvtR.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-Cc2abiRE.js");
...
async function runPluginsDoctorCommand() {
	const { buildPluginCompatibilityNotices, buildPluginDiagnosticsReport, formatPluginCompatibilityNotice } = await import("./status-C-PN3sys.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-Mbk1OVZl.js");
	const { createPluginInstallLogger } = await import("./plugins-command-helpers-B_YfLm_4.js");
