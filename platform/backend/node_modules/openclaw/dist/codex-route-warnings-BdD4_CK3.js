...
import { t as AGENT_MODEL_CONFIG_KEYS } from "./model-refs-Buv1qmKk.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-L6rqw6O0.js";
import { _ as normalizeOptionalAgentRuntimeId, s as openAIProviderUsesCodexRuntimeByDefault } from "./openai-codex-routing-Cc71DT_j.js";
...
import { n as detectWindowsSpawnCommandInlineArgs } from "./windows-spawn-CdrXWu4L.js";
...
function collectAgentRuntimeModelRefs(params) {
	const refs = [];
	const agent = asOptionalRecord(params.agent);
	if (agent && Object.prototype.hasOwnProperty.call(agent, "model")) collectModelConfigRefs({
		refs,
...
function clearStaleSessionRuntimePins(entry) {
	let changed = false;
	if (entry.agentHarnessId !== void 0) {
		delete entry.agentHarnessId;
		changed = true;
	}
	if (entry.agentRuntimeOverride !== void 0) {
...
function scanCodexSessionStoreRoutes(store) {
	return Object.entries(store).flatMap(([sessionKey, entry]) => {
		if (!entry) return [];
		return normalizeOptionalLowercaseString(entry.modelProvider) === "openai-codex" || normalizeOptionalLowercaseString(entry.providerOverride) === "openai-codex" || isOpenAICodexModelRef(entry.model) || isOpenAICodexModelRef(entry.modelOverride) || isOpenAICodexModelRef(entry.fallbackNoticeSelectedModel) || isOpenAICodexModelRef(entry.fallbackNoticeActiveModel) ? [sessionKey] : [];
	});
}
...
//#endregion
export { repairCodexSessionStoreRoutes as i, maybeRepairCodexRoutes as n, maybeRepairCodexSessionRoutes as r, collectCodexRouteWarnings as t };
