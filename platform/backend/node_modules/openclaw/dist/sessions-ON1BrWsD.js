...
import { p as parseStrictNonNegativeInteger } from "./number-coercion-DjYijOzx.js";
import "./parse-finite-number-D6V1jzY2.js";
import { n as buildPluginLoaderJitiOptions, t as buildPluginLoaderAliasMap } from "./sdk-alias-BPp2p_OO.js";
...
import { a as resetOAuthProviders, i as registerOAuthProvider, n as getOAuthProvider, r as getOAuthProviders, t as getOAuthApiKey } from "./oauth-BY5TblPQ.js";
import { t as resolveModelPluginMetadataSnapshot } from "./model-discovery-context-D5OfJKqj.js";
import { a as isGeneratedPluginModelCatalog, i as filterGeneratedPluginModelCatalogProviders, s as listPluginModelCatalogFiles } from "./plugin-model-catalog-ypHZ-GNa.js";
import { _ as getDocsPath, a as getLatestCompactionEntry, b as isBunBinary, c as parseSessionEntries, d as runtime_exports, f as APP_NAME, g as getBinDir, h as getAgentDir, i as getDefaultSessionDir, l as Agent$1, m as VERSION, n as buildSessionContext, o as loadEntriesFromFile, p as CONFIG_DIR_NAME, r as findMostRecentSession, s as migrateSessionEntries, t as SessionManager, u as openClawAgentCoreRuntime, v as getExamplesPath, x as waitForChildProcess, y as getReadmePath } from "./session-manager-Fxwb-hrq.js";
import { c as getSupportedThinkingLevels, l as modelsAreEqual, s as clampThinkingLevel } from "./sanitize-unicode-8KnR9CZq.js";
import { a as resetApiProviders, c as registerApiProvider, i as streamSimple } from "./stream-To8o5poZ.js";
import { t as cleanupSessionResources } from "./session-resources-DwYipzEQ.js";
import { n as getEnvApiKey, t as findEnvKeys } from "./env-api-keys-B9iQniXh.js";
import { B as estimateTokens, F as prepareBranchEntries, G as prepareCompaction$1, H as findTurnStartIndex, I as DEFAULT_COMPACTION_SETTINGS, K as shouldCompact, L as calculateContextTokens, N as collectEntriesForBranchSummaryFromBranches, P as generateBranchSummary$1, R as compact$1, U as generateSummary$1, V as findCutPoint, W as getLastAssistantUsage, q as serializeConversation, tt as convertToLlm, z as estimateContextTokens } from "./proxy-r0kl0JEO.js";
import { t as llm_exports } from "./llm-D4M0S4vd.js";
...
import path, { basename, delimiter, dirname, isAbsolute, join, relative, resolve, sep } from "node:path";
...
//#endregion
//#region src/agents/utils/shell.ts
/**
...
function getShellConfig(customShellPath) {
	if (customShellPath) {
		if (existsSync(customShellPath)) return {
...
function getShellEnv() {
	...
	const updatedPath = currentPath.split(delimiter).filter(Boolean).includes(binDir) ? currentPath : [binDir, currentPath].filter(Boolean).join(delimiter);
	return {
		...process.env,
		[pathKey]: updatedPath
	};
...
//#endregion
//#region src/agents/sessions/tools/truncate.ts
/**
...
function parseGitUrl(source) {
	...
	for (const candidate of hostedCandidates) {
		...
		if (info) {
			...
			if (!isSafeGitHost(host) || !path) continue;
			return {
				type: "git",
...
const IGNORE_FILE_NAMES$1 = [
	".gitignore",
	".ignore",
	".fdignore"
];
function toPosixPath$3(p) {
	return p.split(sep).join("/");
}
function getHomeDir() {
	return process.env.HOME || homedir();
}
function prefixIgnorePattern$1(line, prefix) {
...
function addIgnoreRules$1(ig, dir, rootDir) {
	const relativeDir = relative(rootDir, dir);
	const prefix = relativeDir ? `${toPosixPath$3(relativeDir)}/` : "";
	for (const filename of IGNORE_FILE_NAMES$1) {
		const ignorePath = join(dir, filename);
		if (!existsSync(ignorePath)) continue;
		try {
			const patterns = readFileSync(ignorePath, "utf-8").split(/\r?\n/).map((line) => prefixIgnorePattern$1(line, prefix)).filter((line) => Boolean(line));
...
function collectFiles(dir, filePattern, skipNodeModules = true, ignoreMatcher, rootDir) {
	...
	try {
		...
		for (const entry of entries) {
			...
			const relPath = toPosixPath$3(relative(root, fullPath));
...
function collectAutoPromptEntries(dir) {
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath$3(relative(dir, fullPath));
			if (ig.ignores(relPath)) continue;
...
function matchesAnyPattern(filePath, patterns, baseDir) {
	const rel = toPosixPath$3(relative(baseDir, filePath));
	const name = basename(filePath);
	const filePathPosix = toPosixPath$3(filePath);
	const isSkillFile = name === "SKILL.md";
	const parentDir = isSkillFile ? dirname(filePath) : void 0;
	const parentRel = isSkillFile ? toPosixPath$3(relative(baseDir, parentDir)) : void 0;
	const parentName = isSkillFile ? basename(parentDir) : void 0;
	const parentDirPosix = isSkillFile ? toPosixPath$3(parentDir) : void 0;
	return patterns.some((pattern) => {
		const normalizedPattern = toPosixPath$3(pattern);
...
function normalizeExactPattern(pattern) {
	return toPosixPath$3(pattern.startsWith("./") || pattern.startsWith(".\\") ? pattern.slice(2) : pattern);
}
...
function loadThemeFromPath(themePath, mode) {
	return createTheme(parseThemeJsonContent(themePath, fs$3.readFileSync(themePath, "utf-8")), mode, themePath);
}
...
function createLocalBashOperations(options) {
	return { exec: (command, cwd, { onData, signal, timeout, env }) => {
		return new Promise((resolve, reject) => {
			...
			let timedOut = false;
			let timeoutHandle;
			if (timeout !== void 0 && timeout > 0) timeoutHandle = setTimeout(() => {
				timedOut = true;
...
function createBashToolDefinition(cwd, options) {
	...
	return {
		...
		renderResult(result, options, theme, context) {
			const state = context.state;
			if (state.startedAt !== void 0 && options.isPartial && !state.interval) state.interval = setInterval(() => context.invalidate(), 1e3);
...
function getNoChangeError(path, totalEdits) {
	if (totalEdits === 1) return /* @__PURE__ */ new Error(`No changes made to ${path}. The replacement produced identical content. This might indicate an issue with special characters or the text not existing as expected.`);
	return /* @__PURE__ */ new Error(`No changes made to ${path}. The replacements produced identical content.`);
}
/**
...
async function computeEditsDiff(path, edits, cwd, operations) {
	...
	try {
		...
		const { baseContent, newContent } = applyEditsToNormalizedContent(normalizeToLF(content), edits, path);
...
function formatFindCall(args, theme) {
	...
	const invalidArg = invalidArgText(theme);
	let text = theme.fg("toolTitle", theme.bold("find")) + " " + (pattern === null ? invalidArg : theme.fg("accent", pattern || "")) + theme.fg("toolOutput", ` in ${path === null ? invalidArg : path}`);
...
function formatFindResult(result, options, theme, showImages) {
	...
	if (resultLimit || truncation?.truncated) {
		...
		if (truncation?.truncated) warnings.push(`${formatSize(truncation.maxBytes ?? 51200)} limit`);
					...
					try {
						...
						if (customOps?.glob) {
							...
							const relativized = results.map((p) => {
								if (p.startsWith(searchPath)) return toPosixPath$2(p.slice(searchPath.length + 1));
								return toPosixPath$2(path.relative(searchPath, p));
							});
						...
						child.on("close", (code) => {
							...
							for (const rawLine of lines) {
								...
								relativized.push(toPosixPath$2(relativePath));
							}
							const resultLimitReached = relativized.length >= effectiveLimit;
							const truncation = truncateHead(relativized.join("\n"), { maxLines: Number.MAX_SAFE_INTEGER });
							let resultOutput = truncation.content;
...
function formatGrepCall(args, theme) {
	...
	const limit = args?.limit;
	const invalidArg = invalidArgText(theme);
	let text = theme.fg("toolTitle", theme.bold("grep")) + " " + (pattern === null ? invalidArg : theme.fg("accent", `/${pattern || ""}/`)) + theme.fg("toolOutput", ` in ${path === null ? invalidArg : path}`);
...
//#endregion
//#region src/agents/utils/exif-orientation.ts
function readOrientationFromTiff(bytes, tiffStart) {
	if (tiffStart + 8 > bytes.length) return 1;
	const le = (bytes[tiffStart] << 8 | bytes[tiffStart + 1]) === 18761;
	const read16 = (pos) => {
...
function getExifOrientation(bytes) {
	let tiffOffset = -1;
	if (bytes.length >= 2 && bytes[0] === 255 && bytes[1] === 216) tiffOffset = findJpegTiffOffset(bytes);
	else if (bytes.length >= 12 && bytes[0] === 82 && bytes[1] === 73 && bytes[2] === 70 && bytes[3] === 70 && bytes[8] === 87 && bytes[9] === 69 && bytes[10] === 66 && bytes[11] === 80) tiffOffset = findWebpTiffOffset(bytes);
...
function applyExifOrientation(photon, image, originalBytes) {
	...
	switch (orientation) {
		...
		case 6: return rotate90(photon, image, (x, y, _w, h) => x * h + (h - 1 - y));
		case 7: {
			const rotated = rotate90(photon, image, (x, y, w, h) => (w - 1 - x) * h + y);
			photon.fliph(rotated);
			return rotated;
		}
		case 8: return rotate90(photon, image, (x, y, w, h) => (w - 1 - x) * h + y);
		default: return image;
	}
}
//#endregion
//#region src/agents/utils/photon.ts
const fs$2 = createRequire(import.meta.url)("node:fs");
...
function getFallbackWasmPaths() {
	const execDir = path$1.dirname(process.execPath);
	return [
		path$1.join(execDir, WASM_FILENAME),
		path$1.join(execDir, "photon", WASM_FILENAME),
		path$1.join(process.cwd(), WASM_FILENAME)
	];
}
function patchPhotonWasmRead() {
	const originalReadFileSync = fs$2.readFileSync.bind(fs$2);
	const fallbackPaths = getFallbackWasmPaths();
	const mutableFs = fs$2;
	const patchedReadFileSync = ((...args) => {
		const [file, options] = args;
		if (pathOrNull(file)?.endsWith(WASM_FILENAME)) try {
			return originalReadFileSync(...args);
		} catch (error) {
			const err = error;
			if (err?.code && err.code !== "ENOENT") throw error;
			for (const fallbackPath of fallbackPaths) {
				if (!fs$2.existsSync(fallbackPath)) continue;
				if (options === void 0) return originalReadFileSync(fallbackPath);
	...
	try {
		...
		Object.defineProperty(fs$2, "readFileSync", {
	...
	try {
		...
		function tryEncodings(width, height, jpegQualities) {
			...
			try {
				...
				for (const quality of jpegQualities) candidates.push(encodeCandidate(resized.get_bytes_jpeg(quality), "image/jpeg"));
				return candidates;
			} finally {
				resized.free();
			}
		...
		while (true) {
			...
			for (const candidate of candidates) if (candidate.encodedSize < opts.maxBytes) return {
				data: candidate.data,
			...
			const nextWidth = currentWidth === 1 ? 1 : Math.max(1, Math.floor(currentWidth * .75));
			const nextHeight = currentHeight === 1 ? 1 : Math.max(1, Math.floor(currentHeight * .75));
...
function toPosixPath$1(filePath) {
...
function getOpenClawDocsClassification(absolutePath) {
	...
	const label = toPosixPath$1(relativePath);
				...
				(async () => {
					try {
						...
						if (mimeType) {
							...
							const allLines = (await ops.readFile(absolutePath)).toString("utf-8").split("\n");
...
function resolveExtensionSafeAgentSessionsEntry() {
	...
	return fs$3.existsSync(jsEntry) ? jsEntry : path$1.resolve(currentDirname, "..", "extension-sdk.ts");
}
...
function extensionSourceNeedsJitiAliasResolution(extensionPath) {
	try {
		const source = fs$3.readFileSync(extensionPath, "utf8");
...
async function loadExtensionSourceTransformModule(extensionPath) {
	if (!extensionSourceTransformLoader) extensionSourceTransformLoader = (await loadCreateJitiLoaderFactory())(import.meta.url, {
		...isBunBinary ? {
...
function readResourceManifest(packageJsonPath) {
	try {
		const content = fs$3.readFileSync(packageJsonPath, "utf-8");
...
function resolveExtensionEntries(dir) {
	...
	if (fs$3.existsSync(packageJsonPath)) {
		...
		if (manifest?.extensions?.length) {
			...
			for (const extPath of manifest.extensions) {
				...
				if (fs$3.existsSync(resolvedExtPath)) entries.push(resolvedExtPath);
			}
			if (entries.length > 0) return entries;
		}
	}
	...
	if (fs$3.existsSync(indexTs)) return [indexTs];
	if (fs$3.existsSync(indexJs)) return [indexJs];
	return null;
}
/**
...
async function discoverAndLoadExtensions(configuredPaths, cwd, agentDir = getAgentDir(), eventBus) {
	...
	for (const p of configuredPaths) {
		...
		if (fs$3.existsSync(resolved) && fs$3.statSync(resolved).isDirectory()) {
...
var AgentSession = class {
	...
	*/
	...
	async handleAgentEventUnlocked(event) {
		...
		if (event.type === "message_end") {
			...
			else if (event.message.role === "user" || event.message.role === "assistant" || event.message.role === "toolResult") this.sessionManager.appendMessage(event.message);
	...
	async emitExtensionEvent(event) {
		if (event.type === "agent_start") {
			...
			if (replacement) this.replaceMessageInPlace(event.message, replacement);
	...
	*/
	getLastAssistantText() {
		...
		let text = "";
		for (const content of lastAssistant.content) if (content.type === "text") text += content.text;
...
//#endregion
//#region src/agents/sessions/timings.ts
/**
...
function getAttributionHeaders(model, settingsManager) {
	...
	if (model.provider === "openrouter" || model.baseUrl.includes("openrouter.ai")) return {
		"HTTP-Referer": "https://openclaw.ai",
		"X-OpenRouter-Title": "OpenClaw",
		"X-OpenRouter-Categories": "cli-agent"
	};
	if (model.provider === "cloudflare-workers-ai" || model.provider === "cloudflare-ai-gateway" || model.baseUrl.includes("api.cloudflare.com") || model.baseUrl.includes("gateway.ai.cloudflare.com")) return { "User-Agent": "openclaw" };
}
/**
...
async function createAgentSession(options = {}) {
	...
	if (thinkingLevel === void 0 && hasExistingSession) thinkingLevel = hasThinkingEntry ? existingSession.thinkingLevel : settingsManager.getDefaultThinkingLevel() ?? "medium";
	if (thinkingLevel === void 0) thinkingLevel = settingsManager.getDefaultThinkingLevel() ?? "medium";
...
//#endregion
export { createBashToolDefinition as $, allToolNames as A, truncateTail as At, createWriteToolDefinition as B, isEditToolResult as C, createEventBus as Ct, isReadToolResult as D, formatSize as Dt, isLsToolResult as E, DEFAULT_MAX_LINES as Et, createReadOnlyToolDefinitions as F, createGrepTool as G, createReadToolDefinition as H, createReadOnlyTools as I, createFindToolDefinition as J, createGrepToolDefinition as K, createTool as L, createAllTools as M, createCodingToolDefinitions as N, isToolCallEventType as O, truncateHead as Ot, createCodingTools as P, createBashTool as Q, createToolDefinition as R, isBashToolResult as S, InMemoryAuthStorageBackend as St, isGrepToolResult as T, DEFAULT_MAX_BYTES as Tt, createLsTool as U, createReadTool as V, createLsToolDefinition as W, createEditToolDefinition as X, createEditTool as Y, withFileMutationQueue as Z, loadExtensionFromFactory as _, prepareCompaction as _t, createAgentSessionServices as a, SettingsManager as at, wrapRegisteredTools as b, AuthStorage as bt, loadProjectContextFiles as c, findInitialModel as ct, formatSkillsForPrompt as d, resolveModelScope as dt, createLocalBashOperations as et, loadSkills as f, restoreModelFromSession as ft, discoverAndLoadExtensions as g, generateSummary as gt, createExtensionRuntime as h, compact as ht, createAgentSessionFromServices as i, InMemorySettingsStorage as it, createAllToolDefinitions as j, isWriteToolResult as k, truncateLine as kt, AgentSession as l, parseModelPattern as lt, ExtensionRunner as m, clearApiKeyCache as mt, SessionImportFileNotFoundError as n, createSyntheticSourceInfo as nt, createAgentSession as o, DefaultPackageManager as ot, loadSkillsFromDir as p, ModelRegistry as pt, createFindTool as q, createAgentSessionRuntime as r, FileSettingsStorage as rt, DefaultResourceLoader as s, findExactModelReferenceMatch as st, AgentSessionRuntime as t, createSourceInfo as tt, parseSkillBlock as u, resolveCliModel as ut, loadExtensions as v, collectEntriesForBranchSummary as vt, isFindToolResult as w, executeBashWithOperations as wt, defineTool as x, FileAuthStorageBackend as xt, wrapRegisteredTool as y, generateBranchSummary as yt, createWriteTool as z };
