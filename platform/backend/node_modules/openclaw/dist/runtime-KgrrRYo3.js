...
import { o as resolveAgentWorkspaceDir, r as resolveAgentConfig } from "./agent-scope-config-CaXAl7-j.js";
...
import { d as resolveCommitmentTimezone, f as resolveCommitmentsConfig, i as listPendingCommitmentsForScope, u as upsertInferredCommitments } from "./store-F6p9kRGq.js";
...
function parseCandidate(raw) {
	...
	const source = normalizeOptionalString(raw.source) ?? "inferred_user_context";
	...
	const earliest = normalizeOptionalString(dueWindow?.earliest);
	const latest = normalizeOptionalString(dueWindow?.latest);
...
async function defaultExtractBatch(params) {
	...
	const { runEmbeddedAgent } = await import("./embedded-agent-GbKuappu.js");
	return parseCommitmentExtractionOutput(joinPayloadText(await runEmbeddedAgent({
		sessionId: runId,
