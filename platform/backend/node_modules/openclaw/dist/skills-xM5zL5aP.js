...
import { m as writeJson, t as readDurableJsonFile } from "./json-files-ruLzKqUC.js";
import { c as fetchClawHubSkillDetail, l as fetchClawHubSkillSecurityVerdicts, p as resolveClawHubBaseUrl } from "./clawhub-eHNqclD3.js";
import "./archive-CBe_wA_B.js";
import { Ri as ErrorCodes, an as validateSkillsSecurityVerdictsParams, cn as validateSkillsUpdateParams, dn as validateSkillsUploadCommitParams, in as validateSkillsSearchParams, ln as validateSkillsUploadBeginParams, nn as validateSkillsDetailParams, on as validateSkillsSkillCardParams, rn as validateSkillsInstallParams, sn as validateSkillsStatusParams, t as formatValidationErrors, tn as validateSkillsBinsParams, un as validateSkillsUploadChunkParams, zi as errorShape } from "./src-S72EdpgG.js";
import { s as loadWorkspaceSkillEntries } from "./workspace-CrAPndhs.js";
import { n as redactConfigObject } from "./redact-snapshot-B4Fp0zuB.js";
import { t as canExecRequestNode } from "./exec-defaults-CS74cLqy.js";
import { t as listAgentWorkspaceDirs } from "./workspace-dirs-BWbMuxCp.js";
import { t as getRemoteSkillEligibility } from "./remote-C2XCFMmN.js";
import { c as searchSkillsFromClawHub, f as installSkillArchiveFromPath, p as validateRequestedSkillSlug, r as readLocalSkillCardContentSync, t as installSkillFromClawHub, u as updateSkillsFromClawHub } from "./clawhub-CZ8cBKOU.js";
import { r as updateSkillConfigEntry, t as installSkill } from "./install-CE3uGuCJ.js";
...
const skillsHandlers = {
	...
	"skills.status": ({ params, respond, context }) => {
		...
		respond(true, buildWorkspaceSkillStatus(resolved.workspaceDir, {
			config: resolved.cfg,
	...
	"skills.securityVerdicts": async ({ params, respond, context }) => {
		if (!validateSkillsSecurityVerdictsParams(params)) {
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, `invalid skills.securityVerdicts params: ${formatValidationErrors(validateSkillsSecurityVerdictsParams.errors)}`));
			return;
		}
		const resolved = resolveSkillsAgentWorkspace(params, context);
		if (!resolved.ok) {
		...
		try {
			const targets = collectClawHubVerdictTargets(buildWorkspaceSkillStatus(resolved.workspaceDir, {
				config: resolved.cfg,
				eligibility: { remote: getRemoteSkillEligibility({ advertiseExecNode: canExecRequestNode({
					cfg: resolved.cfg,
					agentId: resolved.agentId
	...
	"skills.skillCard": ({ params, respond, context }) => {
		if (!validateSkillsSkillCardParams(params)) {
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, `invalid skills.skillCard params: ${formatValidationErrors(validateSkillsSkillCardParams.errors)}`));
			return;
		}
		const resolved = resolveSkillsAgentWorkspace(params, context);
		if (!resolved.ok) {
