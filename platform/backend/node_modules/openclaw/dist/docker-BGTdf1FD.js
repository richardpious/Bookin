...
import { m as writeJson } from "./json-files-ruLzKqUC.js";
...
import { s as resolveSandboxHostPathViaExistingAncestor } from "./network-mode-Db8zA3iG.js";
import { i as acquireSessionWriteLock } from "./session-write-lock-5bJqdAc_.js";
import { C as SANDBOX_REGISTRY_PATH, S as SANDBOX_CONTAINERS_DIR, _ as SANDBOX_AGENT_WORKSPACE_MOUNT, b as SANDBOX_BROWSER_REGISTRY_PATH, v as SANDBOX_BROWSERS_DIR } from "./constants-CZs5rTJW.js";
import { n as sanitizeExplicitSandboxEnvVars, t as sanitizeEnvVars } from "./sanitize-env-vars-4wyDg6L9.js";
import { a as resolveWindowsSpawnProgram, r as materializeWindowsSpawnProgram } from "./windows-spawn-CdrXWu4L.js";
import { r as validateSandboxSecurity } from "./validate-sandbox-security-Dh5aBAGT.js";
...
async function readShardedEntry(dir, containerName) {
	let raw;
	try {
		raw = await fs$1.readFile(shardedEntryFilePath(dir, containerName), "utf-8");
	} catch (error) {
...
async function writeShardedEntry(dir, entry) {
	...
	await writeJson(shardedEntryFilePath(dir, entry.containerName), entry, { trailingNewline: true });
}
async function removeShardedEntry(dir, containerName) {
	await fs$1.rm(shardedEntryFilePath(dir, containerName), { force: true });
}
async function readShardedEntries(dir) {
	...
	const entries = await Promise.all(files.filter((name) => name.endsWith(".json")).toSorted().map(async (name) => {
		try {
			return safeParseJsonWithSchema(RegistryEntrySchema, await fs$1.readFile(path.join(dir, name), "utf-8"));
		} catch {
			return null;
		}
	}));
...
async function migrateMonolithicIfNeeded(target) {
	...
	return await withRegistryLock(registryPath, async () => {
		...
		await fs$1.mkdir(shardedDir, { recursive: true });
...
function isExistingWorkspaceSkillMountSource(params) {
	...
	return isPathInside(resolveSandboxHostPathViaExistingAncestor(path.resolve(params.agentWorkspaceDir)), resolveSandboxHostPathViaExistingAncestor(path.resolve(params.hostPath)));
}
...
//#endregion
export { removeRegistryEntry as A, resolveSandboxWorkspaceDir as C, readBrowserRegistry as D, migrateLegacySandboxRegistryFiles as E, updateRegistry as M, computeSandboxBrowserConfigHash as N, readRegistry as O, resolveSandboxScopeKey as S, inspectLegacySandboxRegistryFiles as T, appendWorkspaceMountArgs as _, execDocker as a, resolveReadOnlyWorkspaceSkillMounts as b, isDockerDaemonUnavailable as c, readDockerNetworkDriver as d, readDockerNetworkGateway as f, appendReadOnlyWorkspaceSkillMountArgs as g, resolveDockerSpawnInvocation as h, ensureSandboxContainer as i, updateBrowserRegistry as j, removeBrowserRegistryEntry as k, readDockerContainerEnvVar as l, resolveDockerEnvPolicyEpoch as m, dockerContainerState as n, execDockerRaw as o, readDockerPort as p, ensureDockerImage as r, formatDockerDaemonUnavailableError as s, buildSandboxCreateArgs as t, readDockerContainerLabel as u, formatReadOnlyWorkspaceSkillMountHashState as v, slugifySessionKey as w, resolveSandboxAgentId as x, isExistingWorkspaceSkillMountSource as y };
