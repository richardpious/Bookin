...
import { H as formatMemoryDreamingDay, X as resolveMemoryLightDreamingConfig, Y as resolveMemoryDreamingWorkspaces, Z as resolveMemoryRemDreamingConfig } from "./dreaming-DiJ54ZAu.js";
import { C as parseUsageCountedSessionIdFromFileName } from "./paths-CBJPLq30.js";
import "./security-runtime-CZYvpK-e.js";
import "./string-coerce-runtime-Ccm67Pxa.js";
import "./memory-core-host-status-DqAD6rvR.js";
import { f as loadSessionTranscriptClassificationForAgent, l as buildSessionEntry, m as sessionPathForFile, p as normalizeSessionTranscriptPathForComparison, u as listSessionFilesForAgent } from "./engine-qmd-9ddD0cRD.js";
import "./memory-core-host-engine-qmd-EGAG9h9g.js";
import { n as normalizeTrimmedString } from "./dreaming-shared-twpRe-Pz.js";
import { d as runDetachedDreamNarrative, l as generateAndAppendDreamNarrative } from "./dreaming-narrative-CafEUE2f.js";
import { t as writeDailyDreamingPhaseBlock } from "./dreaming-markdown-zZFhzEqU.js";
import { n as textSimilarity } from "./tokenize-B-rU9X-u.js";
import { d as recordDreamingPhaseSignals, l as readLightStagedKeys, m as recordShortTermRecalls, o as filterLiveShortTermRecallEntries, p as recordRemConsideredPhaseSignals, u as readShortTermRecallEntries } from "./short-term-promotion-BafXEqJ8.js";
...
async function readDailyIngestionState(workspaceDir) {
	try {
		return normalizeDailyIngestionState(await privateFileStore(workspaceDir).readJsonIfExists(DAILY_INGESTION_STATE_RELATIVE_PATH));
	} catch (err) {
...
async function writeDailyIngestionState(workspaceDir, state) {
	await privateFileStore(workspaceDir).writeJson(DAILY_INGESTION_STATE_RELATIVE_PATH, state, { trailingNewline: true });
}
...
async function readSessionIngestionState(workspaceDir) {
	try {
		return normalizeSessionIngestionState(await privateFileStore(workspaceDir).readJsonIfExists(SESSION_INGESTION_STATE_RELATIVE_PATH));
	} catch (err) {
		if (err instanceof SyntaxError) return {
			version: 3,
			files: {},
...
async function writeSessionIngestionState(workspaceDir, state) {
	await privateFileStore(workspaceDir).writeJson(SESSION_INGESTION_STATE_RELATIVE_PATH, state, { trailingNewline: true });
}
...
//#endregion
export { testing as a, seedHistoricalDailyMemorySignals as i, previewRemDreaming as n, runDreamingSweepPhases as r, filterRecallEntriesWithinLookback as t };
