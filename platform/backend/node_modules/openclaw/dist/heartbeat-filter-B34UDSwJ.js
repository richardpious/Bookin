...
import "./heartbeat-tool-response-BO5sRfPm.js";
import { d as stripHeartbeatToken, i as HEARTBEAT_RESPONSE_TOOL_PROMPT, u as resolveHeartbeatPromptForResponseTool } from "./heartbeat-dtJ8QRZo.js";
...
const MESSAGE_TOOL_DELIVERY_PREFIXES = ["Delivery: to send a message, use the `message` tool.", "Delivery: Final assistant text is not automatically delivered in this run. Use the `message` tool to send user-visible output."];
...
function isHeartbeatUserMessage(message, heartbeatPrompt) {
	...
	if (MESSAGE_TOOL_DELIVERY_PREFIXES.some((prefix) => trimmed.startsWith(prefix)) && trimmed.endsWith("[OpenClaw heartbeat poll]")) return true;
