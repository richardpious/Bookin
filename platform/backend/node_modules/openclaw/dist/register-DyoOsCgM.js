...
function policyContainerShapeFindings(policy, policyPath, policyDocName) {
	...
	if (ingressFinding !== void 0) return [ingressFinding];
...
function scopedPolicyShapeFinding(value, params) {
	...
	for (const [scopeName, overlay] of Object.entries(value)) {
		...
		if ((overlay.agents !== void 0 || overlay.tools !== void 0 || overlay.sandbox !== void 0) && !hasAgentIds) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}`, `${params.policyPath} scopes.${scopeName} uses agent-scoped sections without agentIds.`, `List agentIds for agents.workspace, tools, or sandbox policy sections.`);
		const unsupportedKey = Object.keys(overlay).find((key) => key !== "agentIds" && key !== "channelIds" && key !== "agents" && key !== "tools" && key !== "sandbox" && key !== "ingress");
		if (unsupportedKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${ocPathSegment(unsupportedKey)}`, `${params.policyPath} scopes.${scopeName}.${unsupportedKey} is not a supported scoped policy section.`, `Use agentIds with agents.workspace, tools, or sandbox, and channelIds with ingress.channels.`);
...
function toolPosturePolicyShapeFinding(tools, params) {
	...
	const profileAllowFinding = policyStringArrayPropertyShapeFinding((isRecord(tools.profiles) ? tools.profiles : {}).allow, {
	...
	const alsoAllowExpectedFinding = policyStringArrayPropertyShapeFinding((isRecord(tools.alsoAllow) ? tools.alsoAllow : {}).expected, {
		policyDocName: params.policyDocName,
		policyPath: params.policyPath,
...
function bindHostLooksLikeContainerRuntimeSocket(value) {
	...
	return CONTAINER_RUNTIME_SOCKET_PATHS.has(normalized) || CONTAINER_RUNTIME_SOCKET_BASENAMES.has(basename);
}
