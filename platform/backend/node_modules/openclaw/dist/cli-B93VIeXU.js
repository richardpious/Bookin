import { m as parseStrictPositiveInteger } from "./number-coercion-DjYijOzx.js";
import { i as formatErrorMessage } from "./errors-AAwmFBlx.js";
import "./error-runtime-Dn_nurEc.js";
import "./number-runtime-FvL6JHZB.js";
import "./gateway-runtime-CNSNLq8Z.js";
import { n as callGatewayFromCli } from "./gateway-rpc-BHQouYas.js";
import { d as createGoogleMeetSpace, f as endGoogleMeetActiveConference, g as fetchLatestGoogleMeetConferenceRecord, h as fetchGoogleMeetSpace, m as fetchGoogleMeetAttendance, p as fetchGoogleMeetArtifacts, u as buildGoogleMeetPreflightReport } from "./chrome-create-uNzSHn9P.js";
import { n as findGoogleMeetCalendarEvent, r as listGoogleMeetCalendarEvents, t as buildGoogleMeetCalendarDayWindow } from "./calendar-Cjs5KW9S.js";
import { i as exchangeGoogleMeetAuthCode, n as createGoogleMeetOAuthState, o as resolveGoogleMeetAccessToken, r as createGoogleMeetPkce, s as waitForGoogleMeetAuthCode, t as buildGoogleMeetAuthUrl } from "./oauth-Y_3Dm_Vu.js";
import { i as resolveCreateSpaceConfig, r as hasCreateSpaceConfigInput } from "./create-DvAYswIY.js";
...
function isGatewayUnavailableForLocalFallback(err, method) {
	...
	return message.includes("ECONNREFUSED") || message.includes("ECONNRESET") || message.includes("EHOSTUNREACH") || message.includes("ENOTFOUND") || message.includes("gateway not connected") || message.includes(`unknown method: ${method}`);
}
...
function resolveOAuthTokenOptions(config, options) {
	return {
...
async function resolveCalendarMeetingInput(params) {
	if (!hasCalendarLookupOptions(params.options)) return {};
...
async function resolveMeetingForToken(params) {
	...
	const meeting = calendarMeeting.meeting ?? params.configuredMeeting ?? params.config.defaults.meeting;
	...
	return calendarMeeting.calendarEvent ? {
		meeting,
		calendarEvent: calendarMeeting.calendarEvent
	} : { meeting };
}
...
function writeArtifactsSummary(result) {
	...
	for (const entry of result.artifacts) {
		...
		for (const transcript of entry.transcripts) {
			...
			if (transcript.documentTextError) writeStdoutLine("- transcript document body warning: %s", transcript.documentTextError);
		}
		...
		for (const smartNote of entry.smartNotes) {
			...
			if (smartNote.documentTextError) writeStdoutLine("- smart note document body warning: %s", smartNote.documentTextError);
		}
	}
}
...
function csvCell(value) {
	const text = value === void 0 || value === null ? "" : typeof value === "string" || typeof value === "number" || typeof value === "boolean" ? String(value) : JSON.stringify(value);
...
async function writeMeetExportBundle(params) {
	...
	const zipFile = params.zip ? `${outputDir.replace(/\/$/, "")}.zip` : void 0;
	...
	for (const file of files) await writeFile(path.join(outputDir, file.name), file.content, "utf8");
	const result = {
		outputDir,
...
function registerGoogleMeetCli(params) {
	...
	root.command("create").description("Create a new Google Meet space and print its meeting URL").option("--access-token <token>", "Access token override").option("--refresh-token <token>", "Refresh token override").option("--client-id <id>", "OAuth client id override").option("--client-secret <secret>", "OAuth client secret override").option("--expires-at <ms>", "Cached access token expiry as unix epoch milliseconds").option("--access-type <type>", "Google Meet SpaceConfig accessType for API create: OPEN, TRUSTED, or RESTRICTED").option("--entry-point-access <type>", "Google Meet SpaceConfig entryPointAccess for API create: ALL or CREATOR_APP_ONLY").option("--no-join", "Only create the meeting URL; do not join it").option("--transport <transport>", "Join transport: chrome, chrome-node, or twilio").option("--mode <mode>", "Join mode: agent, bidi, or transcribe").option("--message <text>", "Realtime speech to trigger after join").option("--dial-in-number <phone>", "Meet dial-in number for Twilio transport").option("--pin <pin>", "Meet phone PIN; # is appended if omitted").option("--dtmf-sequence <sequence>", "Explicit Twilio DTMF sequence").option("--json", "Print JSON output", false).action(async (options) => {
		if (options.join !== false) {
			...
			if (delegated.ok) {
				...
				if (payload.joined && payload.join?.session?.id) writeStdoutLine("joined: %s", payload.join.session.id);
				else writeStdoutLine("joined: no (run `openclaw googlemeet join %s`)", payload.meetingUri);
				return;
			}
		}
	...
	root.command("end-active-conference").description("End the active conference for a Google Meet space").argument("[meeting]", "Meet URL, meeting code, or spaces/{id}").option("--access-token <token>", "Access token override").option("--refresh-token <token>", "Refresh token override").option("--client-id <id>", "OAuth client id override").option("--client-secret <secret>", "OAuth client secret override").option("--expires-at <ms>", "Cached access token expiry as unix epoch milliseconds").option("--json", "Print JSON output", false).action(async (meeting, options) => {
		const token = await resolveGoogleMeetAccessToken(resolveOAuthTokenOptions(params.config, options));
	...
	root.command("latest").description("Find the latest Meet conference record for a meeting").option("--meeting <value>", "Meet URL, meeting code, or spaces/{id}").option("--today", "Find a Meet link on today's calendar").option("--event <query>", "Find a matching calendar event with a Meet link").option("--calendar <id>", "Calendar id for --today or --event", "primary").option("--access-token <token>", "Access token override").option("--refresh-token <token>", "Refresh token override").option("--client-id <id>", "OAuth client id override").option("--client-secret <secret>", "OAuth client secret override").option("--expires-at <ms>", "Cached access token expiry as unix epoch milliseconds").option("--json", "Print JSON output", false).action(async (options) => {
		const token = await resolveGoogleMeetAccessToken(resolveOAuthTokenOptions(params.config, options));
	...
	root.command("artifacts").description("List Meet conference records and available participant/artifact metadata").option("--meeting <value>", "Meet URL, meeting code, or spaces/{id}").option("--conference-record <name>", "Conference record name or id").option("--today", "Find a Meet link on today's calendar").option("--event <query>", "Find a matching calendar event with a Meet link").option("--calendar <id>", "Calendar id for --today or --event", "primary").option("--access-token <token>", "Access token override").option("--refresh-token <token>", "Refresh token override").option("--client-id <id>", "OAuth client id override").option("--client-secret <secret>", "OAuth client secret override").option("--expires-at <ms>", "Cached access token expiry as unix epoch milliseconds").option("--page-size <n>", "Max resources per Meet API page").option("--all-conference-records", "Fetch every conference record for --meeting").option("--no-transcript-entries", "Skip structured transcript entry lookup").option("--include-doc-bodies", "Export linked transcript and smart-note Google Docs text").option("--format <format>", "Output format: summary or markdown", "summary").option("--output <path>", "Write output to a file instead of stdout").option("--json", "Print JSON output", false).action(async (options) => {
		const resolved = resolveArtifactTokenOptions(params.config, options);
		const token = await resolveGoogleMeetAccessToken(resolved);
		const meeting = resolved.conferenceRecord ? resolved.meeting : (await resolveMeetingForToken({
			config: params.config,
			options,
			accessToken: token.accessToken,
	...
	root.command("attendance").description("List Meet participants and participant sessions").option("--meeting <value>", "Meet URL, meeting code, or spaces/{id}").option("--conference-record <name>", "Conference record name or id").option("--today", "Find a Meet link on today's calendar").option("--event <query>", "Find a matching calendar event with a Meet link").option("--calendar <id>", "Calendar id for --today or --event", "primary").option("--access-token <token>", "Access token override").option("--refresh-token <token>", "Refresh token override").option("--client-id <id>", "OAuth client id override").option("--client-secret <secret>", "OAuth client secret override").option("--expires-at <ms>", "Cached access token expiry as unix epoch milliseconds").option("--page-size <n>", "Max resources per Meet API page").option("--all-conference-records", "Fetch every conference record for --meeting").option("--no-merge-duplicates", "Keep duplicate participant resources as separate rows").option("--late-after-minutes <n>", "Mark participants late after this many minutes", "5").option("--early-before-minutes <n>", "Mark early leavers before this many minutes", "5").option("--format <format>", "Output format: summary, markdown, or csv", "summary").option("--output <path>", "Write output to a file instead of stdout").option("--json", "Print JSON output", false).action(async (options) => {
		const resolved = resolveArtifactTokenOptions(params.config, options);
		const token = await resolveGoogleMeetAccessToken(resolved);
		const meeting = resolved.conferenceRecord ? resolved.meeting : (await resolveMeetingForToken({
			config: params.config,
		...
		if (options.format && options.format !== "summary") throw new Error("Unsupported format. Expected summary, markdown, or csv.");
		writeAttendanceSummary(result);
		writeStdoutLine("token source: %s", token.refreshed ? "refresh-token" : "cached-access-token");
	});
	root.command("export").description("Write Meet artifacts, attendance, transcript, and raw JSON into a folder").option("--meeting <value>", "Meet URL, meeting code, or spaces/{id}").option("--conference-record <name>", "Conference record name or id").option("--today", "Find a Meet link on today's calendar").option("--event <query>", "Find a matching calendar event with a Meet link").option("--calendar <id>", "Calendar id for --today or --event", "primary").option("--access-token <token>", "Access token override").option("--refresh-token <token>", "Refresh token override").option("--client-id <id>", "OAuth client id override").option("--client-secret <secret>", "OAuth client secret override").option("--expires-at <ms>", "Cached access token expiry as unix epoch milliseconds").option("--page-size <n>", "Max resources per Meet API page").option("--all-conference-records", "Fetch every conference record for --meeting").option("--no-transcript-entries", "Skip structured transcript entry lookup").option("--include-doc-bodies", "Export linked transcript and smart-note Google Docs text").option("--no-merge-duplicates", "Keep duplicate participant resources as separate rows").option("--late-after-minutes <n>", "Mark participants late after this many minutes", "5").option("--early-before-minutes <n>", "Mark early leavers before this many minutes", "5").option("--output <dir>", "Output directory").option("--zip", "Also write a portable .zip archive").option("--dry-run", "Fetch export data and print the manifest without writing files", false).option("--json", "Print JSON output", false).action(async (options) => {
		const resolved = resolveArtifactTokenOptions(params.config, options);
		const token = await resolveGoogleMeetAccessToken(resolved);
		const meetingResult = resolved.conferenceRecord ? { meeting: resolved.meeting } : await resolveMeetingForToken({
			config: params.config,
			options,
			accessToken: token.accessToken,
			configuredMeeting: resolved.meeting
		...
		const request = {
			...
			...meetingResult.calendarEvent?.event.id ? { calendarEventId: meetingResult.calendarEvent.event.id } : {},
			...meetingResult.calendarEvent?.event.summary ? { calendarEventSummary: meetingResult.calendarEvent.event.summary } : {},
	...
	root.command("leave").argument("<session-id>", "Meet session ID").action(async (sessionId) => {
		...
		if (delegated.ok) {
			if (!delegated.payload.found) throw new Error("session not found");
	...
	root.command("speak").argument("<session-id>", "Meet session ID").argument("[message]", "Realtime instructions to speak now").action(async (sessionId, message) => {
		...
		const result = await (await params.ensureRuntime()).speak(sessionId, message);
		if (!result.found) throw new Error("session not found");
		if (!result.spoken) throw new Error(result.session?.chrome?.health?.speechBlockedMessage ?? "session has no active realtime audio bridge");
		writeStdoutLine("speaking on %s", sessionId);
	});
