...
import { d as normalizeSecretInputString, u as normalizeResolvedSecretInputString } from "./types.secrets-CuOz9KrZ.js";
import { s as resolveProviderRequestHeaders } from "./provider-request-config-DECw01pI.js";
...
import { i as isProviderAuthProfileConfigured, s as resolveProviderAuthProfileApiKey } from "./provider-auth-DL7jW6qn.js";
import "./secret-input-BDqwHsdT.js";
import "./provider-http-65KY4eRc.js";
import { Q as REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ, Z as REALTIME_VOICE_AUDIO_FORMAT_G711_ULAW_8KHZ } from "./session-log-runtime-1E-x8aMC.js";
import "./realtime-voice-BYkOOxsM.js";
import { a as resolveOpenAIProviderConfigRecord, i as readRealtimeErrorDetail, n as createOpenAIRealtimeClientSecret, o as trimToUndefined, t as captureOpenAIRealtimeWsClose } from "./realtime-provider-shared-_1i7iaXe.js";
...
function prefersCodexOAuthForRealtimeModel(model) {
...
async function resolveOpenAIRealtimeDefaultAuth(params) {
	...
	if (configured.status === "available" || hasOpenAIRealtimeConfiguredApiKeyInput(params.configuredApiKey)) return configured.status === "available" ? {
		status: "available",
		kind: "api-key",
...
async function requireOpenAIRealtimeDefaultAuth(params) {
	...
	if (auth.status === "available") return auth;
...
function hasOpenAIRealtimeBrowserAuthInput(params) {
	...
	if (prefersCodexOAuthForRealtimeModel(params.model)) return isProviderAuthProfileConfigured({
		provider: "openai-codex",
		cfg: params.cfg,
		includeExternalCliAuth: true
	}) || hasOpenAIRealtimeApiKeyInput(void 0);
...
var OpenAIRealtimeVoiceBridge = class OpenAIRealtimeVoiceBridge {
	...
	resolveConnectionParams() {
		...
		if (hasOpenAIRealtimeConfiguredApiKeyInput(cfg.apiKey)) {
			...
			if (directApiKey.status === "missing") throw new Error("OpenAI API key missing");
		...
		if (!isProviderAuthProfileConfigured({
			...
			const directApiKey = resolveOpenAIRealtimeEnvApiKey();
			if (directApiKey.status === "available") return this.resolveApiKeyConnectionParams(directApiKey.value, model);
		}
	...
	async resolveDefaultConnectionParams(model) {
		...
		if (auth.kind === "api-key") return this.resolveApiKeyConnectionParams(auth.value, model);
		if (cfg.azureEndpoint) throw new Error("OpenAI API key missing");
		return this.resolveOAuthConnectionParams(model, auth.value);
	}
	async resolveOAuthConnectionParams(model, authToken) {
		const cfg = this.config;
