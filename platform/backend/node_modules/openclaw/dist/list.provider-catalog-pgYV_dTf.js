...
import { c as resolveProviderOwners, m as loadPluginRegistrySnapshotWithMetadata, s as resolvePluginContributionOwners } from "./plugin-registry-ChAglW1O.js";
import { g as resolveOwningPluginIdsForProviderRef, s as resolveBundledProviderCompatPluginIds } from "./plugin-auto-enable-brBbr1un.js";
import { u as loadAuthProfileStoreWithoutExternalProfiles } from "./store-CvzOxDnC.js";
...
async function resolveProviderCatalogPluginIdsForFilter(params) {
	...
	const { resolveProviderContractPluginIdsForProviderAlias } = await import("./registry-D4xIJ4LM.js");
...
async function loadProviderCatalogModelsForList(params) {
	...
	for (const order of DISCOVERY_ORDERS) for (const provider of byOrder[order] ?? []) {
		...
		let result;
		try {
			if (params.staticOnly === true || typeof provider.staticCatalog?.run === "function") result = await runProviderStaticCatalog({
				provider,
				config: params.cfg,
				...
				result = await runProviderCatalog({
					...
					agentDir: params.agentDir,
					env,
					resolveProviderApiKey: (providerId) => resolveProviderApiKey(providerId?.trim() || provider.id),
					resolveProviderAuth: (providerId, options) => resolveProviderAuth(providerId?.trim() || provider.id, options)
				});
			}
		} catch (error) {
...
//#endregion
export { loadProviderCatalogModelsForList as n, resolveProviderCatalogPluginIdsForFilter as r, hasProviderStaticCatalogForFilter as t };
