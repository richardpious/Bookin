...
import { p as projectProviderCatalogResultToUnifiedTextRows } from "./loader-wybWjJVr.js";
...
function resolveEnvVars(params) {
	const combined = normalizeStringEntries([...params.envVars ?? [], ...(params.auth ?? []).map((entry) => entry.envVar).filter(Boolean)]);
