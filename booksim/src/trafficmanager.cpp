...
TrafficManager::TrafficManager(const Configuration &config,
    ...
    : Module(0, "traffic_manager"), _net(net), _empty_network(false),
  ...
  if (packet_size_rate_str.empty()) {
    ...
    vector<string> packet_size_rate_strings =
        tokenize_str(packet_size_rate_str);
  ...
  for (int c = 0; c < _classes; ++c) {
    ...
    _injection_process[c] =
        InjectionProcess::New(injection_process[c], _nodes, _load[c], &config);
  }
  ...
  for (int source = 0; source < _nodes; ++source) {
    ...
    for (int subnet = 0; subnet < _subnets; ++subnet) {
      ...
      int router_latency =
          config.GetInt("routing_delay") +
          (config.GetInt("speculative") ? max(vc_alloc_delay, sw_alloc_delay)
  ...
  if (outstanding_credits_out_file == "") {
    ...
    _outstanding_credits_out =
        new ofstream(outstanding_credits_out_file.c_str());
  }
  ...
  if (used_credits_out_file == "") {
    ...
    _used_credits_out = new ofstream(used_credits_out_file.c_str());
      if (_used_credits_out) *_used_credits_out << "Credits Used per VC\n";
  }
  ...
  if (free_credits_out_file == "") {
    ...
    _free_credits_out = new ofstream(free_credits_out_file.c_str());
      if (_free_credits_out) *_free_credits_out << "Credits Free per Port\n";
  }
  ...
  if (max_credits_out_file == "") {
    ...
    _max_credits_out = new ofstream(max_credits_out_file.c_str());
      if (_max_credits_out) *_max_credits_out << "Max Credits per Port\n";
  }
#endif
...
void TrafficManager::_RetireFlit(Flit *f, int dest) {
  ...
  if (f->tail) {
    ...
    if ((_sim_state == warming_up) || f->record) {
      ...
      _frag_stats[f->cl]->AddSample((f->atime - head->atime) -
                                    (f->id - head->id));

      if (_pair_stats) {
        _pair_plat[f->cl][f->src * _nodes + dest]->AddSample(f->atime -
                                                             head->ctime);
        _pair_nlat[f->cl][f->src * _nodes + dest]->AddSample(f->atime -
                                                             head->itime);
      }
    }
...
void TrafficManager::_Step() {
  ...
  /*Checking for Deadlock if there is any flits in flight and the deadlock timer
  value exceeds the timeout set in the config (deadlock_warn_timeout in
  booksim_config.cpp) */
  ...
  for (int subnet = 0; subnet < _subnets; ++subnet) {
    for (int n = 0; n < _nodes; ++n) {
      ...
      if (c) {
#ifdef TRACK_FLOWS
        for (set<int>::const_iterator iter = c->vc.begin(); iter != c->vc.end();
             ++iter) {
          int const vc = *iter;
    ...
    for (int n = 0; n < _nodes; ++n) {
      ...
      for (int i = 1; i <= class_limit; ++i) {
        ...
        if (cf->head && cf->vc == -1) { // Find first available VC
          ...
          OutputSet route_set;
          _rf(NULL, cf, -1, &route_set,
              true); // NUll: We are routing at the injection ; cf: Current
                     // Flit; -1: No input port(injection); true: routing for
                     // injection
          set<OutputSet::sSetElement> const &os =
              route_set.GetSet(); // Get the set of possible ouput ports and VCs
          assert(os.size() == 1); // it should be one since it's injection
...
void TrafficManager::_ComputeStats(const vector<int> &stats, int *sum, int *min,
                                   int *max, int *min_pos, int *max_pos) const {
  ...
  while ((total_phases < _max_samples) &&
    ...
    for (int c = 0; c < _classes; ++c) {
      ...
      double total_accepted_rate =
          (double)total_accepted_count / (double)(_time - _reset_time);
      double cur_accepted = total_accepted_rate / (double)_nodes;

      double latency_change =
          fabs((cur_latency - prev_latency[c]) / cur_latency);
      prev_latency[c] = cur_latency;

      double accepted_change =
          fabs((cur_accepted - prev_accepted[c]) / cur_accepted);
      ...
      for (iter = _total_in_flight_flits[c].begin();
           iter != _total_in_flight_flits[c].end(); iter++) {
        latency += (double)(_time - iter->second->ctime);
        count++;
      }

      if ((lat_exc_class < 0) && (_latency_thres[c] >= 0.0) &&
      ...
      if (lat_chg_exc_class < 0) {
        if ((_sim_state == warming_up) && (_warmup_threshold[c] >= 0.0) &&
            (latency_change > _warmup_threshold[c])) {
          lat_chg_exc_class = c;
        } else if ((_sim_state == running) && (_stopping_threshold[c] >= 0.0) &&
      ...
      if (acc_chg_exc_class < 0) {
        if ((_sim_state == warming_up) && (_acc_warmup_threshold[c] >= 0.0) &&
    ...
    if (_measure_latency && (lat_exc_class >= 0)) {
      ...
      cout << "Average latency for class " << lat_exc_class << " exceeded "
           << _latency_thres[lat_exc_class] << " cycles. Aborting simulation."
           << endl;
      converged = 0;
      _sim_state = draining;
      _drain_time = _time;
...
// active_packets: header will be printed per class for readability
...
#endif
#ifdef TRACK_STALLS
        _buffer_busy_stalls[c][subnet * _routers + router] +=
            r->GetBufferBusyStalls(c);
        _buffer_conflict_stalls[c][subnet * _routers + router] +=
            r->GetBufferConflictStalls(c);
        _buffer_full_stalls[c][subnet * _routers + router] +=
            r->GetBufferFullStalls(c);
        _buffer_reserved_stalls[c][subnet * _routers + router] +=
            r->GetBufferReservedStalls(c);
        _crossbar_conflict_stalls[c][subnet * _routers + router] +=
            r->GetCrossbarConflictStalls(c);
        r->ResetStallStats(c);
#endif
      }
    }
  ...
  for (int s = 0; s < _subnets; ++s) {
    ...
    for (int n = 0; n < _nodes; ++n) {
      ...
      if(_used_credits_out) *_used_credits_out << " Node " << n << ": [";
      if(_free_credits_out) *_free_credits_out << " Node " << n << ": [";
      if(_max_credits_out) *_max_credits_out << " Node " << n << ": [";
...
void TrafficManager::DisplayStats(ostream &os) const {
  ...
  for (int c = 0; c < _classes; ++c) {
    ...
    cout << "Injected packet length average = "
         << (double)sent_flits / (double)sent_packets << endl
         << "Accepted packet length average = "
         << (double)accepted_flits / (double)accepted_packets << endl;

    cout << "Total in-flight flits = " << _total_in_flight_flits[c].size()
         << " (" << _measured_in_flight_flits[c].size() << " measured)" << endl;
...
string TrafficManager::_OverallStatsCSV(int c) const {
  ostringstream os;
  os << _traffic[c] << ',' << _use_read_write[c] << ',' << _load[c] << ','
