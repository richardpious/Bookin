...
VCDTracer::VCDTracer(Configuration const &config, int nodes, int routers,
                     int router_outputs, int vcs)
    : _enabled(config.GetInt("vcd_trace") > 0),
      ...
      _trace_packet(config.GetInt("vcd_trace_packet")), _nodes(nodes),
      _routers(routers), _router_outputs(router_outputs), _vcs(vcs),
...
bool VCDTracer::Enabled() const { return _enabled; }

bool VCDTracer::InTraceWindow(int cycle) const {
  return _enabled && (cycle >= _start_cycle) &&
         ((_end_cycle < 0) || (cycle <= _end_cycle));
}
...
void VCDTracer::TracePacketGenerated(int node, int packet_id, int src, int dest,
  ...
  if (_trace_flit >= 0 &&
      (_trace_flit < flit_id_start || _trace_flit > flit_id_end)) {
    return;
  }
  _Trace(_node_gen[node], _node_gen_valid_last[node], packet_id, src, dest,
         flit_id_start, flit_id_end);
}
...
void VCDTracer::TraceChannelBegin(std::string const &, int source_router,
                                  int source_port, int sink_router, int, int,
                                  Flit const *f) {
  if (!InTraceWindow(GetSimTime()) || !ShouldTrace(f)) {
    return;
  }
  if (source_router < 0) {
    int node = f->src;
  ...
  if (source_router >= _routers || source_port < 0 ||
      source_port >= _router_outputs) {
    return;
  }
  if (!ShouldTraceRouter(source_router))
    return;
  _Trace(_router_link[source_router][source_port],
         _router_link_valid_last[source_router][source_port], f);

  if (_trace_credits && !_router_ds_occupancy.empty()) {
    int vc = f->vc;
    if (vc >= 0 && vc < _vcs) {
      int count = --_router_ds_occupancy_count[source_router][source_port][vc];
      if (f->watch) {
        *gWatchOut << GetSimTime()
                   << " | VCD_DEBUG | Decrementing occupancy count to " << count
                   << " for router " << source_router << " port " << source_port
                   << " vc " << vc << std::endl;
      }
...
void VCDTracer::TraceChannelEnd(std::string const &, int, int, int, int,
                                Flit const *) {}
...
void VCDTracer::TraceVCState(int router, int input, int vc, int state,
  ...
  if (!_trace_vc || !InTraceWindow(GetSimTime()))
    return;
  if (router < 0 || router >= _routers || input < 0 ||
      input >= _router_outputs || vc < 0 || vc >= _vcs)
    return;
  if (!ShouldTraceRouter(router))
    return;
  if (_router_vc_signals.empty())
    return;
...
void VCDTracer::TracePipelineVA(int router, int input, int vc, Flit const *f,
                                int result, int out_port, int out_vc_val) {
  if (!_trace_pipeline || !InTraceWindow(GetSimTime()))
    return;
  if (router < 0 || router >= _routers || input < 0 || input >= _router_outputs)
    return;
...
void VCDTracer::TracePipelineSA(int router, int input, int vc, Flit const *f,
                                int result, int out_port) {
  if (!_trace_pipeline || !InTraceWindow(GetSimTime()))
    return;
  if (router < 0 || router >= _routers || input < 0 || input >= _router_outputs)
    return;
...
void VCDTracer::TracePipelineST(int router, int input, int output,
                                Flit const *f, bool begin) {
  if (!_trace_pipeline || !InTraceWindow(GetSimTime()))
    return;
  if (router < 0 || router >= _routers || input < 0 || input >= _router_outputs)
    return;
  ...
  if (begin) {
    ...
    if (_trace_credits && !_router_ds_occupancy.empty()) {
      int vc = f->vc;
      if (vc >= 0 && vc < _vcs) {
        int count = ++_router_ds_occupancy_count[router][output][vc];
        if (f->watch) {
          *gWatchOut << GetSimTime()
                     << " | VCD_DEBUG | Incrementing occupancy count to "
                     << count << " for router " << router << " port " << output
                     << " vc " << vc << std::endl;
        }
...
void VCDTracer::TraceDownstreamCredits(int router, int output, int vc,
                                       int occupancy, int available,
                                       int limit) {
  if (!_trace_credits || !InTraceWindow(GetSimTime()))
    return;
  if (router < 0 || router >= _routers || output < 0 ||
      output >= _router_outputs || vc < 0 || vc >= _vcs)
    return;
  if (!ShouldTraceRouter(router))
    return;
  if (_router_ds_occupancy.empty())
    return;

  // Occupancy is now tracked by ST and ChannelBegin phases, ignoring the
  // credit-based occupancy
...
void VCDTracer::_WriteHeader() {
  ...
  for (int node = 0; node < _nodes; ++node) {
    ...
    _node_gen[node].flit_id_start =
        _RegisterInteger(prefix.str() + ".flit_id_start", 32);
    _node_gen[node].flit_id_end =
        _RegisterInteger(prefix.str() + ".flit_id_end", 32);
  }
  ...
  for (int router = 0; router < _routers; ++router) {
    ...
    if (_trace_credits && trace_this) {
      ...
      _router_ds_occupancy_count[router].resize(_router_outputs);
    }
    ...
    for (int output = 0; output < _router_outputs; ++output) {
      ...
      if (trace_this) {
        ...
        for (int vc = 0; vc < _vcs; ++vc) {
          ...
          _router_vc_occupancy[router][output][vc] =
              _RegisterInteger(vc_prefix.str() + ".occupancy", 32);
        }
        ...
        if (_trace_vc) {
          ...
          for (int vc = 0; vc < _vcs; ++vc) {
            std::ostringstream vc_prefix;
            vc_prefix << "router_" << router << ".in_" << output << ".vc_"
                      << vc;
            _router_vc_signals[router][output][vc].state =
            ...
            _router_vc_signals[router][output][vc].out_vc =
                _RegisterInteger(vc_prefix.str() + ".out_vc", 8);
          }
        }
        ...
        _router_in[router][output].valid =
            _Register(in_prefix.str() + ".valid", 1);
        _router_in[router][output].flit =
            _RegisterInteger(in_prefix.str() + ".flit_id", 32);
        _router_in[router][output].packet =
            _RegisterInteger(in_prefix.str() + ".packet_id", 32);
        _router_in[router][output].vc =
            _RegisterInteger(in_prefix.str() + ".vc", 32);
        _router_in[router][output].src =
            _RegisterInteger(in_prefix.str() + ".src", 32);
        _router_in[router][output].dest =
            _RegisterInteger(in_prefix.str() + ".dest", 32);
        _router_in[router][output].head =
            _Register(in_prefix.str() + ".head", 1);
        _router_in[router][output].tail =
            _Register(in_prefix.str() + ".tail", 1);

        // Router link (output) signals
        std::ostringstream prefix;
        prefix << "router_" << router << ".link_" << output;
        _router_link[router][output].valid =
            _Register(prefix.str() + ".valid", 1);
        _router_link[router][output].flit =
            _RegisterInteger(prefix.str() + ".flit_id", 32);
        _router_link[router][output].packet =
            _RegisterInteger(prefix.str() + ".packet_id", 32);
        _router_link[router][output].vc =
            _RegisterInteger(prefix.str() + ".vc", 32);
        _router_link[router][output].src =
            _RegisterInteger(prefix.str() + ".src", 32);
        _router_link[router][output].dest =
            _RegisterInteger(prefix.str() + ".dest", 32);
        _router_link[router][output].head =
            _Register(prefix.str() + ".head", 1);
        ...
        if (_trace_credits) {
          ...
          _router_ds_occupancy_count[router][output].resize(_vcs, 0);
          for (int vc = 0; vc < _vcs; ++vc) {
            std::ostringstream ds_prefix;
            ds_prefix << "router_" << router << ".ds.out_" << output << ".vc_"
                      << vc;
            _router_ds_occupancy[router][output][vc] =
                _RegisterInteger(ds_prefix.str() + ".occupancy", 8);
            _router_ds_available[router][output][vc] =
                _RegisterInteger(ds_prefix.str() + ".available", 8);
          }
        }
      } else {
        // Non-traced router: register dummy signals (needed for consistent
        // indexing)
        for (int vc = 0; vc < _vcs; ++vc) {
          std::ostringstream vc_prefix;
          vc_prefix << "router_" << router << ".in_" << output << ".vc_" << vc;
...
void VCDTracer::_Set(std::string const &id, unsigned long long value) {
  if (id.empty())
    return;
  std::unordered_map<std::string, unsigned long long>::iterator it =
      _last_values.find(id);
...
void VCDTracer::_Trace(PacketGenSignals const &sigs, char &valid_last,
                       int packet_id, int src, int dest, int flit_id_start,
                       int flit_id_end) {
  _SetBit(sigs.valid, true);
  valid_last = true;
